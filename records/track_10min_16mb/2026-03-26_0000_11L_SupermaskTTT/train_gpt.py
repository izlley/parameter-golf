...
def eval_val_sliding_ttt(
    ...
    """Supermask TTT: freeze all model weights, learn per-MLP channel masks.
    ...
    # --- Supermask setup: channel-level masks on MLP hidden dims ---
    # Freeze ALL model weights
    for p in base_model.parameters():
        p.requires_grad_(False)

    # Create mask scores for each MLP block (init=5.0 → sigmoid≈0.993, nearly identity)
    mask_scores: list[Tensor] = []
    original_forwards: list = []
    for block in base_model.blocks:
        hidden_dim = block.mlp.fc.weight.shape[0]
        ms = torch.full((hidden_dim,), 5.0, device=device, requires_grad=True)
        mask_scores.append(ms)
        original_forwards.append(block.mlp.forward)

    # Monkey-patch MLP forwards with masked versions
    for block, ms in zip(base_model.blocks, mask_scores):
        fc_layer = block.mlp.fc
        proj_layer = block.mlp.proj
        def _make_masked_fwd(fc, proj, mask_s):
            def masked_forward(x):
                h = F.leaky_relu(fc(x), negative_slope=0.5)
                h = h.square()
                mask = torch.sigmoid(mask_s).to(h.dtype)
                h = h * mask  # channel-level mask [hidden] broadcasts to [B,T,hidden]
                return proj(h)
            return masked_forward
        block.mlp.forward = _make_masked_fwd(fc_layer, proj_layer, ms)
    ...
    log0(f"supermask_ttt:params mask_params={total_mask_params} model_frozen={total_frozen}")
    ...
    # Restore original MLP forwards and requires_grad
    for block, orig_fwd in zip(base_model.blocks, original_forwards):
