...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def _learnable_clip_per_row(t32: Tensor, clip_range: int, lr: float = 0.02, steps: int = 80) -> tuple[Tensor, Tensor]:
    """OmniQuant-style Learnable Weight Clipping: optimize per-row clipping bounds via gradient descent.
    Instead of searching over 5 percentile candidates, learn the optimal per-row scale
    that minimizes reconstruction MSE using STE (straight-through estimator)."""
    rows, cols = t32.shape
    # Initialize from best percentile search (warm start)
    row_amax = t32.abs().amax(dim=1)
    # Learnable log-scale: s = exp(log_s) * row_amax / clip_range (ensures positivity)
    log_s = torch.zeros(rows, device=t32.device, dtype=torch.float32, requires_grad=True)
    optimizer = torch.optim.Adam([log_s], lr=lr)
    for _ in range(steps):
        optimizer.zero_grad()
        s = (row_amax * torch.exp(log_s) / clip_range).clamp_min(1.0 / clip_range)
        # STE: forward uses round, backward passes through
        q_float = torch.clamp(t32 / s[:, None], -clip_range, clip_range)
        q_round = q_float + (torch.round(q_float) - q_float).detach()
        recon = q_round * s[:, None]
        loss = (t32 - recon).pow(2).mean()
        loss.backward()
        optimizer.step()
    # Final quantize with optimized scales
    with torch.no_grad():
        s_final = (row_amax * torch.exp(log_s.detach()) / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
        q = torch.clamp(torch.round(t32 / s_final.float()[:, None]), -clip_range, clip_range).to(torch.int8)
    return q, s_final
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        return _learnable_clip_per_row(t32, clip_range)
