...
# --- QuaRot-style Hadamard rotation for quantization ---
...
def _hadamard_matrix(n: int) -> Tensor:
    ...
    H is orthogonal and self-inverse: H @ H = I."""
    ...
    if n == 1:
        ...
        H = torch.cat([top, bot], dim=0) / math.sqrt(2)
    _HADAMARD_CACHE[n] = H
    return H

def _rotate_columns(W: Tensor) -> Tensor:
    """Apply Hadamard rotation to columns (input dim) of a 2D weight matrix.
    For power-of-2 column count, applies full Hadamard.
    For non-power-of-2, applies block-diagonal Hadamard in chunks of the
    largest power-of-2 that divides the column count.
    Returns W_rot = W @ H (block-diagonal)."""
    if W.ndim != 2:
        return W
    rows, cols = W.shape
    # Find largest power-of-2 that divides cols
    blk = 1
    while blk * 2 <= cols and cols % (blk * 2) == 0:
        blk *= 2
    if blk < 4:  # Too small to benefit from rotation
        return W
    H = _hadamard_matrix(blk).to(W.device, dtype=W.dtype)
    n_blocks = cols // blk
    if n_blocks == 1:
        return W @ H
...
def _unrotate_columns(W: Tensor) -> Tensor:
    """Inverse of _rotate_columns: W_orig = W_rot @ H^T = W_rot @ H (self-inverse)."""
    # For normalized Walsh-Hadamard, H^T = H (symmetric and orthogonal), so inverse = same op
    return _rotate_columns(W)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    quarot_stats = {"rotated": 0, "not_rotated": 0}
    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            # QuaRot: apply Hadamard rotation to columns before quantization
            rotated = False
            quant_fn = quantize_int5_per_row if layer_idx in int5_layers else quantize_int6_per_row
            q_orig, s_orig = quant_fn(t)
            if t.ndim == 2:
                t_rot = _rotate_columns(t.float())
                q_rot, s_rot = quant_fn(t_rot)
                # Compare errors in original space
                recon_orig = q_orig.float() * s_orig.float().view(q_orig.shape[0], *([1] * (q_orig.ndim - 1)))
                err_orig = (t.float() - recon_orig).pow(2).mean().item()
                recon_rot = q_rot.float() * s_rot.float().view(q_rot.shape[0], *([1] * (q_rot.ndim - 1)))
                recon_rot_orig = _unrotate_columns(recon_rot)
                err_rot = (t.float() - recon_rot_orig).pow(2).mean().item()
                if err_rot < err_orig:
                    q_orig, s_orig = q_rot, s_rot
                    rotated = True
            qtype = "int5" if layer_idx in int5_layers else "int6"
            result[name + ".q"] = q_orig
            result[name + ".scale"] = s_orig
            meta[name] = {"type": qtype, "rotated": rotated}
            if rotated:
                quarot_stats["rotated"] += 1
            else:
                quarot_stats["not_rotated"] += 1
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    print(f"QuaRot: {quarot_stats['rotated']} tensors rotated, {quarot_stats['not_rotated']} kept original")
    return result, meta
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        # QuaRot: undo Hadamard rotation if it was applied during quantization
        if isinstance(info, dict) and info.get("rotated", False) and w.ndim == 2:
            w = _unrotate_columns(w)
