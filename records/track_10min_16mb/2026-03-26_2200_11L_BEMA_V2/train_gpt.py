...
def main() -> None:
    ...
    ema_decay = 0.9995  # BEMA V2: higher decay so bias correction is meaningful
    ema_step = 0  # BEMA: track number of EMA updates for bias correction
    ...
    while True:
        ...
        for opt in optimizers:
            opt.step()
        zero_grad_all()
        # EMA update (BEMA: bias-corrected)
        ema_step += 1
        with torch.no_grad():
    ...
    # Apply BEMA V2 (bias-corrected EMA with higher decay)
    # With decay=0.9995 and ~6300 steps: 0.9995^6300 ≈ 0.042
    # correction = 1/(1-0.042) ≈ 1.044 — meaningful bias correction
    bema_correction = 1.0 / (1.0 - ema_decay ** ema_step)
    log0(f"bema_v2:applying bias-corrected EMA weights (ema_step={ema_step}, decay={ema_decay}, correction={bema_correction:.6f})")
    current_state = base_model.state_dict()
    avg_state = {name: (t * bema_correction).to(dtype=current_state[name].dtype) for name, t in ema_state.items()}
