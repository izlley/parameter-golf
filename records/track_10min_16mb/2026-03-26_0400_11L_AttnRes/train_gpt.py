...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,pseudo_query",
    ).split(",")
    if pattern
)
...
class AttnResOp(nn.Module):
    """Attention Residual operator: learned softmax attention over depth.
    Replaces fixed residual accumulation with weighted combination of all
...
class GPT(nn.Module):
    def __init__(
        ...
        self.attn_res_enabled = attn_res
        if attn_res:
...
def main() -> None:
    ...
    for ar_op in base_model.attn_res_ops:
        scalar_params.append(ar_op.pseudo_query)
    ...
    log0(f"attn_res:{'enabled' if args.attn_res else 'disabled'} ops:{len(base_model.attn_res_ops)} params:{sum(p.numel() for op in base_model.attn_res_ops for p in op.parameters())}")
    log0(
