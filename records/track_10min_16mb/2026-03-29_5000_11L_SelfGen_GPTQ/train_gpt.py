...
def generate_calibration_data(
    ...
    """Generate calibration sequences using the model itself (AR self-generation).

    Instead of using validation data (which risks data leakage), the model
    generates its own calibration data for GPTQ quantization.
    """
    model.eval()
    all_sequences: list[Tensor] = []
    with torch.no_grad(), torch.amp.autocast("cuda", dtype=torch.bfloat16):
        for batch_start in range(0, num_sequences, batch_size):
            cur_batch = min(batch_size, num_sequences - batch_start)
            tokens = torch.randint(0, vocab_size, (cur_batch, 1), device=device)
            for _ in range(seq_len - 1):
                # Use only last 512 tokens as context to keep generation fast
                ctx = tokens[:, -512:]
    ...
    return torch.cat(all_sequences, dim=0)  # (num_sequences, seq_len)
def collect_gptq_hessian(
    model: nn.Module,
    calibration_data: Tensor,
    device: torch.device,
    batch_size: int = 8,
) -> dict[str, Tensor]:
    """Collect Hessian diagonal (X^T X) for each Linear layer using calibration data.

    Returns dict mapping parameter name -> Hessian diagonal tensor (on CPU).
    """
    model.eval()
    hessian_accum: dict[str, Tensor] = {}
    ...
    for name, module in model.named_modules():
        if isinstance(module, (nn.Linear, CastedLinear)):
            ...
            name_map[id(module)] = param_name
            def make_hook(mod_id: int, pname: str):
                def hook_fn(mod, inp, out):
                    ...
                    x_f = x.float()
                    # Diagonal of X^T X
                    diag = (x_f * x_f).sum(dim=0)  # (D,)
                    if pname not in hessian_accum:
                        hessian_accum[pname] = torch.zeros_like(diag)
                    hessian_accum[pname] += diag
                    sample_counts[pname] = sample_counts.get(pname, 0) + x.shape[0]
                return hook_fn
            h = module.register_forward_hook(make_hook(id(module), param_name))
            hooks.append(h)
    # Run calibration data through the model
    num_seqs = calibration_data.shape[0]
    with torch.no_grad(), torch.amp.autocast("cuda", dtype=torch.bfloat16):
        for i in range(0, num_seqs, batch_size):
            batch = calibration_data[i:i + batch_size].to(device)
            # Use forward_logits to get a full forward pass
    ...
    for pname, diag in hessian_accum.items():
        n = sample_counts.get(pname, 1)
        result[pname] = (diag / n).cpu()
    return result
def gptq_quantize_int6(weight: Tensor, hessian_diag: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """GPTQ-style int6 quantization with Hessian-weighted error compensation.

    Quantizes column-by-column, compensating remaining columns for quantization error
    weighted by the Hessian diagonal (importance of each input dimension).
    """
    W = weight.float().clone()
    rows, cols = W.shape
    # Per-row scale based on max absolute value
    ...
    H_diag = hessian_diag.float().to(W.device)
    H_diag = H_diag.clamp_min(1e-8)
    # Process columns in order of importance (highest Hessian first for stability)
    col_order = torch.argsort(H_diag, descending=True)
    for idx in range(cols):
        col = col_order[idx].item()
        # Quantize this column
        q_col = torch.clamp(torch.round(W[:, col] / scale_f), -clip_range, clip_range).to(torch.int8)
        Q[:, col] = q_col
        # Compute quantization error
        err = W[:, col] - q_col.float() * scale_f
        # Compensate remaining unquantized columns proportional to Hessian weight
        if idx < cols - 1:
            remaining = col_order[idx + 1:]
            # Weight compensation by relative Hessian importance
            h_remaining = H_diag[remaining]
            h_col = H_diag[col]
            # Distribute error to remaining columns, weighted by their importance ratio
            comp_weights = h_col / (h_remaining + 1e-8)
            comp_weights = comp_weights.clamp_max(1.0)  # Don't over-compensate
            W[:, remaining] += err[:, None] * comp_weights[None, :] / len(remaining)
    return Q, scale
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
                        hessian_dict: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            # Use GPTQ quantization if Hessian available for this layer
            if hessian_dict is not None and name in hessian_dict and t.ndim == 2:
                q, s = gptq_quantize_int6(t, hessian_dict[name])
            else:
...
def main() -> None:
    ...
    log0("gptq:generating self-calibration data (64 sequences)...")
    t_selfgen = time.perf_counter()
    if master_process:
        calib_data = generate_calibration_data(
            base_model, device, num_sequences=64, seq_len=1024,
            temperature=0.8, vocab_size=args.vocab_size, batch_size=8,
        )
        log0(f"gptq:self-gen done shape={tuple(calib_data.shape)} "
             f"elapsed={time.perf_counter() - t_selfgen:.1f}s")
    else:
        calib_data = torch.zeros((64, 1024), dtype=torch.long, device=device)
    if distributed:
        dist.broadcast(calib_data, src=0)
    log0("gptq:collecting Hessian from calibration data...")
    t_hess = time.perf_counter()
    hessian_dict = collect_gptq_hessian(base_model, calib_data, device, batch_size=8)
    log0(f"gptq:hessian done layers={len(hessian_dict)} "
         f"elapsed={time.perf_counter() - t_hess:.1f}s")
    del calib_data
    torch.cuda.empty_cache()
    # --- End GPTQ Calibration ---
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, hessian_dict=hessian_dict)
    del hessian_dict
