...
class Hyperparameters:
    ...
    cerwu_alpha = float(os.environ.get("CERWU_ALPHA", 0.001))
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def cerwu_loss(model: nn.Module, clip_range: int = 31) -> Tensor:
    """CERWU: penalize distance of weights from their nearest quantization level.
    Encourages weights to snap to the int6 grid -> lower entropy -> better zstd compression."""
    ...
    for p in model.parameters():
        ...
        w_scaled = w / scale
        w_rounded = torch.round(w_scaled).detach()
...
def main() -> None:
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
            # CERWU: add entropy regularization during warmdown
            if args.cerwu_alpha > 0 and scale < 1.0:
                cerwu = cerwu_loss(base_model)
                loss = loss + args.cerwu_alpha * cerwu
