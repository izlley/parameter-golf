...
class CastedLinear(nn.Linear):
    ...
    _hadamard_qat: bool = False  # Hadamard-rotated int4 QAT for MLP
    _hadamard_cache: dict = {}  # Cache Hadamard matrices by size

    @staticmethod
    def _get_hadamard(n: int, device: torch.device) -> Tensor:
        key = (n, device)
        if key not in CastedLinear._hadamard_cache:
            # Pad to nearest power of 2 if needed
            n_pad = 1
            while n_pad < n:
                n_pad *= 2
            CastedLinear._hadamard_cache[key] = hadamard_matrix(n_pad).to(device)[:n, :n]
        return CastedLinear._hadamard_cache[key]

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if CastedLinear._hadamard_qat and self.training and w.ndim == 2 and getattr(self, '_use_hadamard', False):
            # Hadamard-rotated int4 QAT
            with torch.no_grad():
                w32 = self.weight.float()
                H = self._get_hadamard(w32.shape[1], w32.device).to(w32.dtype)
                w_rot = w32 @ H  # Rotate weights
                # int4 quantization: clip_range=7 (4-bit signed: -8 to 7)
                row_max = w_rot.abs().amax(dim=1)
                scale = (row_max / 7.0).clamp_min(1.0 / 7.0)
                w_q_rot = (torch.clamp(torch.round(w_rot / scale[:, None]), -8, 7) * scale[:, None])
                w_q = (w_q_rot @ H.T).to(x.dtype)  # Rotate back
            w = w + (w_q - w).detach()  # STE
        elif CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                w32 = self.weight.float()
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.fc._use_hadamard = True  # Mark for Hadamard int4 QAT
        self.proj = CastedLinear(hidden, dim, bias=False)
        self.proj._zero_init = True
        self.proj._use_hadamard = True  # Mark for Hadamard int4 QAT
    def forward(self, x: Tensor) -> Tensor:
...
def quantize_int4_per_row(t: Tensor, clip_range: int = 7) -> tuple[Tensor, Tensor]:
    """Quantize to int4 range [-8, 7] with per-row scaling and MSE-optimal clipping."""
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        ...
        if best_q is None:
            ...
            best_q = torch.zeros(t32.shape[0], t32.shape[1], dtype=torch.int8)
            best_s = s
        return best_q, best_s
    amax = t32.abs().max().item()
    scale = torch.tensor(amax / clip_range if amax > 0 else 1.0, dtype=torch.float16)
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            if cat == "mlp" and t.ndim == 2:
                # Hadamard-rotated int4 for MLP
                H = hadamard_matrix(t.shape[1]).to(t.device)[:t.shape[1], :t.shape[1]]
                t_rot = t.float() @ H
                q, s = quantize_int4_per_row(t_rot)
                result[name + ".q"] = q
                result[name + ".scale"] = s
                result[name + ".hadamard_cols"] = torch.tensor(t.shape[1], dtype=torch.int32)
                meta[name] = {"type": "int4_hadamard"}
            else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        if isinstance(info, dict) and info.get("type") == "int4_hadamard":
            q_key, s_key = name + ".q", name + ".scale"
            h_key = name + ".hadamard_cols"
            if q_key not in result or s_key not in result:
                continue
            q, s = result[q_key], result[s_key]
            if q is None or s is None:
                continue
            n_cols = int(result[h_key].item())
            H = hadamard_matrix(n_cols)[:n_cols, :n_cols]
            if s.ndim > 0:
                t_rot = (q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1))))
            else:
                t_rot = q.float() * float(s.item())
            out[name] = (t_rot @ H.T).to(orig_dtype)  # Rotate back
            continue
...
def main() -> None:
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            CastedLinear._hadamard_qat = True
