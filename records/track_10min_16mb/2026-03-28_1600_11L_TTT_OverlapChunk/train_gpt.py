...
def eval_val_sliding_ttt(
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            base_model.train()
            # Overlap: include latter 50% of previous chunk for training
            if ci > 0:
                overlap_start = max(0, (ci - 1) * ttt_chunk + ttt_chunk // 2)
            else:
                overlap_start = chunk_start
            train_start = overlap_start
            train_end = chunk_end
            chunk_seqs = (train_end - train_start) // seq_len
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        start_tok = train_start + actual_bs * seq_len
                        end_tok = train_start + (my_seq_s + be) * seq_len + 1
