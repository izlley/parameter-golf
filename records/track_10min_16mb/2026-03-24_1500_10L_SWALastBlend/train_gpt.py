...
class Hyperparameters:
    ...
    swa_blend_ratio = float(os.environ.get("SWA_BLEND_RATIO", 0.6))  # SWA weight in blend (1-ratio = last weight)
...
def main() -> None:
    ...
    # Apply SWA-Last Blend if collected
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        swa_ratio = args.swa_blend_ratio
        last_ratio = 1.0 - swa_ratio
        log0(f"swa_blend:applying {swa_count} checkpoints, swa_ratio:{swa_ratio} last_ratio:{last_ratio}")
        # Save last checkpoint state
        last_state = {name: t.detach().cpu().clone() for name, t in base_model.state_dict().items()}
        # Compute SWA average
        current_state = base_model.state_dict()
        blended_state = {}
        for name, tensor in swa_state.items():
            swa_avg = tensor / swa_count
            last_val = last_state[name]
            blended = swa_ratio * swa_avg + last_ratio * last_val
            blended_state[name] = blended.to(dtype=current_state[name].dtype)
        base_model.load_state_dict(blended_state, strict=True)
