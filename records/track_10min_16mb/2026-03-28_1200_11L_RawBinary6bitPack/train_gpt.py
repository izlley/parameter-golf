...
def pack_int6_to_bytes(q: Tensor) -> bytes:
    ...
    # Convert signed (-31..31) to unsigned (0..62)
    ...
    # Pack 4 values into 3 bytes: [a b c d] -> 3 bytes
...
def serialize_raw_binary(quant_result: dict, quant_meta: dict) -> bytes:
    ...
    for name in sorted_names:
        ...
        # Write dtype code: 0=int8, 1=float16, 2=float32, 3=int6_packed, 4=int32, 5=int64
        shape = list(t.shape)
        numel = t.numel()
        # Check if this is an int6 quantized tensor (name ends with .q and meta says int6)
        ...
        if is_int6_q:
            ...
            if t.dtype == torch.int8:
                ...
                dtype_code = 2  # fallback to float32
                t = t.float()
            buf.write(struct.pack('<B', dtype_code))
            buf.write(struct.pack('<B', len(shape)))
            for s in shape:
