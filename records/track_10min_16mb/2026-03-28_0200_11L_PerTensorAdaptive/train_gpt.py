...
def quantize_int5_per_row(t: Tensor, clip_range: int = 15) -> tuple[Tensor, Tensor]:
    ...
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    return q, scale
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], target_size: int = 15_800_000):
    num_layers_total = max(
        (int(k.split(".")[1]) for k in state_dict if k.startswith("blocks.")),
        default=0,
    ) + 1
    # Phase 1: Quantize all as int6 first, collect candidates for int5 downgrade
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    candidates: list[tuple[str, float, Tensor, Tensor]] = []  # (name, sensitivity, q5, s5)
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            t32 = t.float()
            # Default: quantize as int6
            q6, s6 = quantize_int6_per_row(t)
            result[name + ".q"] = q6
            result[name + ".scale"] = s6
            meta[name] = {"type": "int6"}
            # Compute int5 quantization for sensitivity comparison
            ...
            if s6.ndim > 0:
                ...
                recon6 = q6.float() * float(s6.item())
                recon5 = q5.float() * float(s5.item())
            err6 = (t32 - recon6).pow(2).mean().item()
            err5 = (t32 - recon5).pow(2).mean().item()
            sensitivity = err5 - err6  # Lower = less sensitive = better candidate for int5
            candidates.append((name, sensitivity, q5, s5))
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    # Phase 2: Sort by sensitivity (ascending), apply int5 to least sensitive ~40%
    candidates.sort(key=lambda x: x[1])
    num_to_convert = len(candidates) * 2 // 5  # Convert ~40% least sensitive
    for i in range(num_to_convert):
        name, _, q5, s5 = candidates[i]
        result[name + ".q"] = q5
...
def main() -> None:
    ...
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
    # EWQ: mark early layer CastedLinear modules for int5 QAT (adaptive: 5 layers approx)
    for i in range(min(5, len(base_model.blocks))):
