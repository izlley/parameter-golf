...
class Hyperparameters:
    ...
    qat_ramp_steps = int(os.environ.get("QAT_RAMP_STEPS", 2000))
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._qat and self.training and w.ndim == 2:
            ...
            w_q = q * scale
            # Progressive QAT: alpha ramps 0→1, blending unquantized→quantized
            alpha = self._qat_alpha  # tensor buffer, updated in training loop
            w = w + alpha * (w_q - w_f).detach()  # Progressive STE
...
def main() -> None:
    ...
    if args.qat_enabled:
        for name, module in base_model.named_modules():
            if isinstance(module, CastedLinear):
                ...
                # Progressive QAT: alpha as tensor buffer (torch.compile safe)
                module.register_buffer("_qat_alpha", torch.tensor(0.0, device=device), persistent=False)
        if master_process:
            log0(f"qat:progressive enabled, ramp 0→1 over {args.qat_ramp_steps} steps (int5 MLP, int6 attn)")
    ...
    while True:
        ...
        # Progressive QAT: ramp alpha from 0 to 1
        if args.qat_enabled and args.qat_ramp_steps > 0:
            qat_alpha = min(step / args.qat_ramp_steps, 1.0)
            for module in base_model.modules():
                if isinstance(module, CastedLinear) and hasattr(module, "_qat_alpha"):
                    module._qat_alpha.fill_(qat_alpha)
        ...
        if should_log_train:
            ...
            if master_process and _HAS_WANDB:
                log_dict = {"train_loss": tl, "step_avg_ms": approx_training_time_ms / step, "lr_scale": scale}
                if args.qat_enabled:
                    log_dict["qat_alpha"] = min(step / max(args.qat_ramp_steps, 1), 1.0)
