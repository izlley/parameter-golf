...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,smear,dtg_gate,ve_layer_scales,ve_shared.scale,depth_router,iter_scales",
    ).split(",")
    if pattern
)
...
class DepthRouter(nn.Module):
    ...
    def __init__(self, dim: int, hidden: int = 32):
        ...
        nn.init.constant_(self.out.bias, 0.0)  # 50% routing initially
    def forward(self, x: Tensor) -> Tensor:
        # x: (B, T, D) -> (B, T, 1) probability of using iteration 2
        h = F.gelu(self.fc(x))
        return torch.sigmoid(self.out(h))  # (B, T, 1)
class GPT(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        ...
        # MoR: share block 5 and 6 (middle layers)
        self.mor_layer_idx = 5  # layer 5 is the shared block, iterated twice (effective layer 5 + 6)
        ...
        # MoR: block 6 shares weights with block 5
        self.blocks[self.mor_layer_idx + 1] = self.blocks[self.mor_layer_idx]
        # Router and per-iteration scaling
        self.depth_router = DepthRouter(model_dim, hidden=32)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        num_layers = len(self.blocks)
        for i in range(num_layers):
            if i == self.mor_layer_idx:
                # MoR: iteration 1
                ve = self._get_ve(i, input_ids, ve_cache)
                x_iter1 = self.blocks[i](x, x0, v_embed=ve)
                ...
                # Iteration 2 (always computed for torch.compile, masked by router)
                ve2 = self._get_ve(i + 1, input_ids, ve_cache)
                x_iter2 = self.blocks[i](x_iter1, x0, v_embed=ve2)
                x_iter2 = x_iter2 * self.iter_scales[1].to(dtype=x.dtype)[None, None, :]
                # Blend: route_prob=1 → use iter2, route_prob=0 → use iter1
                x = route_prob * x_iter2 + (1 - route_prob) * x_iter1
            elif i == self.mor_layer_idx + 1:
                # Skip: already handled by MoR above (shared block)
                pass
            else:
                ve = self._get_ve(i, input_ids, ve_cache)
