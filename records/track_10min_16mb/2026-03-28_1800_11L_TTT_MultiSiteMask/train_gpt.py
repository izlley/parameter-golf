...
def eval_val_sliding_ttt(
    ...
    # --- Supermask V3 setup: masks + biases on MLP hidden + Attn output + QKV + gate ---
    ...
    #            qkv_mask(q_dim+k_dim+v_dim), gate_bias(dim)
    mask_scores: list[Tensor] = []
    original_mlp_forwards: list = []
    original_attn_forwards: list = []
    original_cq_forwards: list = []
    original_ck_forwards: list = []
    original_cv_forwards: list = []
    for block in base_model.blocks:
        ...
        # NEW: QKV channel mask — per output channel for q, k, v projections
        q_dim = block.attn.c_q.weight.shape[0]
        k_dim = block.attn.c_k.weight.shape[0]
        v_dim = block.attn.c_v.weight.shape[0]
        qkv_ms = torch.full((q_dim + k_dim + v_dim,), 3.0, device=device, requires_grad=True)
        # NEW: Gate bias — additional multiplicative gate on attention output (init=3.0 → ~0.95)
        gate_bias = torch.full((attn_dim,), 3.0, device=device, requires_grad=True)
        mask_scores.extend([mlp_ms, mlp_bias, attn_ms, attn_bias, qkv_ms, gate_bias])
        original_mlp_forwards.append(block.mlp.forward)
        original_attn_forwards.append(block.attn.forward)
        original_cq_forwards.append(block.attn.c_q.forward)
        original_ck_forwards.append(block.attn.c_k.forward)
        original_cv_forwards.append(block.attn.c_v.forward)

    # Monkey-patch forwards with masked+biased versions
    for bi, block in enumerate(base_model.blocks):
        mlp_ms = mask_scores[bi * 6]        # MLP mask
        mlp_bias = mask_scores[bi * 6 + 1]  # MLP bias
        attn_ms = mask_scores[bi * 6 + 2]   # Attn mask
        attn_bias = mask_scores[bi * 6 + 3] # Attn bias
        qkv_ms = mask_scores[bi * 6 + 4]    # QKV mask
        gate_bias = mask_scores[bi * 6 + 5] # Gate bias
        ...
        # NEW: Monkey-patch c_q, c_k, c_v with QKV channel masks
        q_dim = block.attn.c_q.weight.shape[0]
        k_dim = block.attn.c_k.weight.shape[0]
        def _make_masked_linear_fwd(orig_fwd, ms_slice):
            def masked_forward(x):
                out = orig_fwd(x)
                mask = torch.sigmoid(ms_slice).to(out.dtype)
                return out * mask
            return masked_forward
        block.attn.c_q.forward = _make_masked_linear_fwd(original_cq_forwards[bi], qkv_ms[:q_dim])
        block.attn.c_k.forward = _make_masked_linear_fwd(original_ck_forwards[bi], qkv_ms[q_dim:q_dim + k_dim])
        block.attn.c_v.forward = _make_masked_linear_fwd(original_cv_forwards[bi], qkv_ms[q_dim + k_dim:])

        # Monkey-patch Attn forward: apply channel mask + bias + gate to output
        orig_attn_fwd = block.attn.forward.__func__ if hasattr(block.attn.forward, '__func__') else None
        def _make_masked_attn_fwd(attn_module, ms, bias, gb, orig_fwd):
            def masked_attn_forward(x, v_embed=None):
                out = orig_fwd(attn_module, x, v_embed=v_embed)
                mask = torch.sigmoid(ms).to(out.dtype)
                out = out * mask + bias.to(out.dtype)
                # Additional learned gate on attention output
                gate = torch.sigmoid(gb).to(out.dtype)
                out = out * gate
                return out
            return masked_attn_forward
        if orig_attn_fwd is not None:
            block.attn.forward = _make_masked_attn_fwd(block.attn, attn_ms, attn_bias, gate_bias, orig_attn_fwd)
    ...
    log0(f"supermask_ttt:params mask_params={total_mask_params} "
         f"(blocks={num_blocks} per_block=mlp_mask+mlp_bias+attn_mask+attn_bias+qkv_mask+gate_bias) model_frozen={total_frozen}")
    ...
    # Restore original MLP, Attn, and QKV forwards and requires_grad
    for block, orig_mlp_fwd, orig_attn_fwd, orig_cq, orig_ck, orig_cv in zip(
        base_model.blocks, original_mlp_forwards, original_attn_forwards,
        original_cq_forwards, original_ck_forwards, original_cv_forwards,
    ):
        block.mlp.forward = orig_mlp_fwd
        block.attn.forward = orig_attn_fwd
        block.attn.c_q.forward = orig_cq
        block.attn.c_k.forward = orig_ck
        block.attn.c_v.forward = orig_cv
    for p in base_model.parameters():
