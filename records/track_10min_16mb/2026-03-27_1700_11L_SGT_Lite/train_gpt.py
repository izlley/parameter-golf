...
class Hyperparameters:
    ...
    # SGT Lite: fixed attention looping on specified layer
    sgt_loop_layer = int(os.environ.get("SGT_LOOP_LAYER", 10))  # which layer to loop (-1 = disabled)
    sgt_loop_warmup = int(os.environ.get("SGT_LOOP_WARMUP", 500))  # steps before enabling loop
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,loop_attn_scale",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        return x_out
    def loop_attn(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        """Re-run attention on block output (SGT lite: attention-only, no MLP re-run).
        Returns the attention output (caller applies loop_attn_scale and adds to x)."""
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_layers: str = "9,10",
        gated_attention: bool = False,
        sgt_loop_layer: int = -1,
    ):
        super().__init__()
        self.sgt_loop_layer = sgt_loop_layer
        ...
        # SGT Lite: learnable scale for loop attention (init zeros = no loop effect at start)
        if sgt_loop_layer >= 0:
            self.loop_attn_scale = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))
        else:
            self.loop_attn_scale = None
        self._init_weights()
    ...
    def _maybe_loop_attn(self, layer_idx: int, x: Tensor, ve: Tensor | None) -> Tensor:
        """Apply SGT loop attention on target layer if enabled."""
        if self.loop_attn_scale is not None and layer_idx == self.sgt_loop_layer:
            loop_out = self.blocks[layer_idx].loop_attn(x, v_embed=ve)
            x = x + self.loop_attn_scale.to(dtype=x.dtype)[None, None, :] * loop_out
        return x
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self._maybe_loop_attn(i, x, ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self._maybe_loop_attn(bi, x, ve)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        sgt_loop_layer=args.sgt_loop_layer,
    ).to(device).bfloat16()
    ...
    if base_model.loop_attn_scale is not None:
        scalar_params.append(base_model.loop_attn_scale)
    ...
    # SGT Lite: log config (loop_attn_scale init=zeros → gradual activation)
    if args.sgt_loop_layer >= 0:
        log0(f"sgt_lite:enabled loop_layer:{args.sgt_loop_layer}")
