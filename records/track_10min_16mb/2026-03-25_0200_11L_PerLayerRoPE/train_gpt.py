...
def flash_attn_3_func(q, k, v, **kwargs):
    """FA3 wrapper with SDPA fallback. FA3 expects (B,T,H,D), SDPA expects (B,H,T,D)."""
    if _USE_FA3:
        return _fa3_func(q, k, v, **kwargs)
    q2 = q.transpose(1, 2)
    k2 = k.transpose(1, 2)
    v2 = v.transpose(1, 2)
...
class Hyperparameters:
    ...
    # Per-layer RoPE dims: "32,32,32,32,16,16,16,16,0,0,0" (shallow=more RoPE, deep=NoPE)
    per_layer_rope_dims = os.environ.get("PER_LAYER_ROPE_DIMS", "32,32,32,32,16,16,16,16,0,0,0")
...
class GPT(nn.Module):
    def __init__(
        ...
        if per_layer_rope_dims:
            layer_rope_list = [int(x.strip()) for x in per_layer_rope_dims.split(",")]
            assert len(layer_rope_list) == num_layers, f"per_layer_rope_dims length {len(layer_rope_list)} != num_layers {num_layers}"
            for i, block in enumerate(self.blocks):
                rd = layer_rope_list[i]
                block.attn.rope_dims = rd
                if rd > 0:
                    block.attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024, rope_dims=rd)
                else:
                    # NoPE: rope_dims=0 means no rotary applied (apply_rotary_emb is identity when rope_dims=0)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        rope_dims=args.rope_dims, per_layer_rope_dims=args.per_layer_rope_dims,
    ...
    per_layer_rd = [b.attn.rope_dims for b in base_model.blocks]
    log0(f"per_layer_rope_dims:{per_layer_rd}")
    log0(
    ...
    eval_model = GPT(
        ...
        rope_dims=args.rope_dims, per_layer_rope_dims=args.per_layer_rope_dims, ln_scale=args.ln_scale, dtg=args.dtg_enabled,
