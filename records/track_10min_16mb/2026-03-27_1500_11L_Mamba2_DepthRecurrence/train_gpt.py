...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 10))  # 9 attn + 1 Mamba2 (looped 2x = effective 11)
    ...
    ve_layers = os.environ.get("VE_LAYERS", "7,8")  # adjusted: layer 9 is Mamba2 (no VE)
    # Mamba2 SSM config
    mamba_layers = os.environ.get("MAMBA_LAYERS", "9")  # comma-separated layer indices (last block)
    ...
    mamba_loop_count = int(os.environ.get("MAMBA_LOOP_COUNT", 2))  # repeat Mamba2 block N times
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,ssm_scale,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,dt_bias,A_log,mamba_iter_embed",
    ).split(",")
    if pattern
)
...
class Mamba2Block(nn.Module):
    """Block with Mamba2 SSM + MLP. Supports looping via external iteration."""
...
class GPT(nn.Module):
    def __init__(
        ...
        # Parse Mamba layer indices
        mamba_layer_set = set(int(x) for x in mamba_layers.split(",") if x.strip())
        self.mamba_layer_set = mamba_layer_set
        self.mamba_loop_count = mamba_loop_count
        ...
        # Iteration embeddings for Mamba2 loop (one per loop iteration)
        if mamba_loop_count > 1 and mamba_layer_set:
            self.mamba_iter_embed = nn.Parameter(torch.zeros(mamba_loop_count, model_dim))
        else:
            self.mamba_iter_embed = None
        if rope_dims > 0:
    ...
    def _run_block(self, bi: int, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict) -> Tensor:
        """Run a single block, with Mamba2 looping if configured."""
        block = self.blocks[bi]
        if bi in self.mamba_layer_set and self.mamba_loop_count > 1:
            # Loop the Mamba2 block multiple times with iteration embeddings
            for loop_i in range(self.mamba_loop_count):
                if self.mamba_iter_embed is not None:
                    x = x + self.mamba_iter_embed[loop_i].to(dtype=x.dtype)[None, None, :]
                x = block(x, x0)
        else:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mamba_loop_count=args.mamba_loop_count,
    ).to(device).bfloat16()
    ...
    if base_model.mamba_iter_embed is not None:
        scalar_params.append(base_model.mamba_iter_embed)
