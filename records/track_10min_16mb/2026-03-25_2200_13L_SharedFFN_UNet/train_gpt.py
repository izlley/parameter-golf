...
class Hyperparameters:
    ...
    shared_ffn_groups = int(os.environ.get("SHARED_FFN_GROUPS", 6))  # 6 pairs + 1 independent
...
class GPT(nn.Module):
    def __init__(
        ...
        if use_shared:
            ...
            for i in range(num_layers):
                if i < shared_ffn_groups * 2:
                    self._layer_to_mlp_group.append(i // 2)
                else:
                    self._layer_to_mlp_group.append(-1)  # independent
        else:
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    shared_mlp=(use_shared and i < shared_ffn_groups * 2),
    ...
    def _get_shared_mlp(self, layer_idx: int) -> nn.Module | None:
        """Return the shared MLP for this layer, or None if not using shared FFN or independent."""
        if not self._layer_to_mlp_group:
            return None
        group = self._layer_to_mlp_group[layer_idx]
        if group < 0:
            return None
        return self.shared_mlps[group]
