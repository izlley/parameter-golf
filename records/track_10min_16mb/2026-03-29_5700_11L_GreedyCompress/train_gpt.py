...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 11))  # XSA on ALL layers (abaybektursun PR#1019)
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Quantize to [-clip_range, clip_range]. Works for int6 (31), int5 (15), etc."""
    t32 = t.float()
    if t32.ndim == 2:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    return quantize_int_per_row(t, clip_range)
def _measure_compressed_size(q: Tensor, s: Tensor) -> int:
    """Measure actual zstd-compressed size of a quantized tensor pair."""
    buf = io.BytesIO()
    torch.save({"q": q, "s": s}, buf)
    raw = buf.getvalue()
    compressed = zstandard.ZstdCompressor(level=22).compress(raw) if _COMPRESSOR == "zstd" else zlib.compress(raw, 9)
    return len(compressed)
def _compute_quant_mse(t: Tensor, clip_range: int) -> float:
    """Compute MSE of quantizing tensor t at given clip_range."""
    t32 = t.float()
    if t32.ndim != 2:
        return 0.0
    q, s = quantize_int_per_row(t32, clip_range)
    recon = q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
    return (t32 - recon).pow(2).mean().item()
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
                        target_size: int = 16_000_000, code_bytes: int = 0):
    """Greedy Iterative Compression: start all-int6, downgrade tensors to int5
    one at a time (cheapest first) until total artifact fits target_size."""
    # Phase 1: Quantize everything at int6 (or int8 for non-int6 cats)
    quantizable: list[str] = []  # names of tensors eligible for int5 downgrade
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    tensors_fp: dict[str, Tensor] = {}  # original fp tensors for re-quantization
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    # Phase 2: Measure current total compressed size
    ...
    current_size = len(quant_blob) + code_bytes
    print(f"greedy_compress: initial_size={current_size} target={target_size} "
          f"candidates={len(quantizable)}")
    if current_size <= target_size:
        print("greedy_compress: already within target, no downgrade needed")
        return result, meta
    # Phase 3: Pre-compute cost/benefit for each candidate tensor
    # cost = MSE(int5) - MSE(int6) (quality degradation)
    # benefit = compressed_size(int6) - compressed_size(int5) (size saving)
    candidates = []
    for name in quantizable:
        t = tensors_fp[name]
        mse_int6 = _compute_quant_mse(t, 31)
        mse_int5 = _compute_quant_mse(t, 15)
        cost = (mse_int5 - mse_int6) * t.numel()  # total error increase
        # Estimate size saving: int5 values are in [-15,15] vs [-31,31]
        # Actual saving depends on zstd — estimate from value range reduction
        q5, s5 = quantize_int_per_row(t, 15)
        q6 = result[name + ".q"]
        s6 = result[name + ".scale"]
        size_int6 = _measure_compressed_size(q6, s6)
        size_int5 = _measure_compressed_size(q5, s5)
        saving = size_int6 - size_int5
        if saving <= 0:
            continue  # int5 doesn't save space for this tensor
        ratio = cost / saving  # lower = cheaper to downgrade
        candidates.append((name, ratio, saving, cost, q5, s5))
        print(f"  candidate: {name} cost={cost:.6f} saving={saving} ratio={ratio:.8f}")
    # Sort by ratio (cheapest first)
    candidates.sort(key=lambda x: x[1])
    # Phase 4: Greedily downgrade until we fit
    downgraded = []
    for name, ratio, saving, cost, q5, s5 in candidates:
        result[name + ".q"] = q5
        result[name + ".scale"] = s5
        meta[name] = {"type": "int5"}
        downgraded.append(name)
        # Re-measure total size
        quant_buf = io.BytesIO()
        torch.save({"w": result, "m": meta}, quant_buf)
        quant_raw = quant_buf.getvalue()
        quant_blob = zstandard.ZstdCompressor(level=22).compress(quant_raw) if _COMPRESSOR == "zstd" else zlib.compress(quant_raw, 9)
        current_size = len(quant_blob) + code_bytes
        print(f"greedy_compress: downgraded {name} -> int5, "
              f"size={current_size} ratio={ratio:.8f}")
        if current_size <= target_size:
            print(f"greedy_compress: TARGET REACHED after {len(downgraded)} downgrades")
            break
    if current_size > target_size:
        print(f"greedy_compress: WARNING still over target by {current_size - target_size} bytes "
              f"after {len(downgraded)} downgrades")
    print(f"greedy_compress: downgraded_tensors={downgraded}")
    return result, meta
...
def main() -> None:
    ...
    # Heuristic int5 QAT for early layers (greedy compress will refine at export time)
    ewq_int5_layers = min(6, len(base_model.blocks))
    for i in range(ewq_int5_layers):
        for m in base_model.blocks[i].modules():
            if isinstance(m, CastedLinear):
                m._ewq_int5 = True
    log0(f"ewq_int5_qat: layers 0-{ewq_int5_layers - 1} marked for int5 QAT")
