...
class GPT(nn.Module):
    def __init__(
        ...
        # MoR: Layer 8 and 9 share the same block (recursive)
        self.mor_layer_idx = 8  # Layer 8 is the recursive block
        self.mor_max_repeat = 2  # Max 2 iterations
        blocks: list[nn.Module] = []
        for i in range(num_layers):
            if i == self.mor_layer_idx + 1:
                # Layer 9 shares Layer 8's block (recursive)
                blocks.append(blocks[self.mor_layer_idx])
            else:
                blocks.append(Block(
                    model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                    layer_idx=i, ln_scale=ln_scale, dtg=dtg, gated_attention=gated_attention,
                ))
        self.blocks = nn.ModuleList(blocks)
        # Router for adaptive depth
        self.depth_router = DepthRouter(model_dim)
        # Per-iteration scale for the recursive block (iteration embedding)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if bi == self.mor_layer_idx:
                # MoR recursive block: iteration 1
                ...
                # Iteration 2 (always computed for torch.compile compat, masked by router)
                x_iter2 = self.blocks[bi](x_iter1, x0, v_embed=ve)
