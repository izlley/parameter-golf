...
def main() -> None:
    ...
    if master_process:
        log0("temperature_search:start")
    best_temp = 1.0
    best_temp_bpb = q_val_bpb
    for temp in [0.92, 0.94, 0.96, 0.98, 1.02, 1.04, 1.06, 1.08]:
        t_loss, t_bpb = eval_val_sliding(
            args, base_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, batch_seqs=args.eval_batch_seqs, temperature=temp,
        )
        log0(f"temperature:{temp:.2f} val_bpb:{t_bpb:.6f}")
        if t_bpb < best_temp_bpb:
            best_temp = temp
            best_temp_bpb = t_bpb
    log0(f"best_temperature:{best_temp:.2f} best_val_bpb:{best_temp_bpb:.6f}")

    if master_process and _HAS_WANDB:
        wandb.log({"final_val_loss": q_val_loss, "final_val_bpb": q_val_bpb,
                    "best_temperature": best_temp, "best_temp_bpb": best_temp_bpb,
