...
class Hyperparameters:
    ...
    entropy_reg_lambda = float(os.environ.get("ENTROPY_REG_LAMBDA", 0.0001))
    entropy_reg_start_frac = float(os.environ.get("ENTROPY_REG_START_FRAC", 0.7))
...
def main() -> None:
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            # Compression-friendly L1 regularization (ramp up during warmdown)
            entropy_reg_start_step = int(args.entropy_reg_start_frac * args.iterations)
            if step >= entropy_reg_start_step and step % 10 == 0:
                l1_penalty = sum(
                    p.abs().mean() for name, p in base_model.named_parameters()
                    if p.ndim == 2 and p.numel() > 65536
                    and not any(pat in name for pat in CONTROL_TENSOR_NAME_PATTERNS)
                )
                total_warmdown = max(args.iterations - entropy_reg_start_step, 1)
                progress = min((step - entropy_reg_start_step) / total_warmdown, 1.0)
                loss = loss + args.entropy_reg_lambda * progress * l1_penalty
