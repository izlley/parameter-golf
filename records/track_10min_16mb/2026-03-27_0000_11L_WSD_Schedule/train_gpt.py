...
def main() -> None:
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        """WSD Schedule: Warmup → Stable → sharp linear Decay.
        warmdown_iters controls the final decay phase length."""
        ...
        # Wallclock-based: estimate warmdown start from remaining time
        step_ms = elapsed_ms / max(step, 1)
        warmdown_ms = args.warmdown_iters * step_ms
        remaining_ms = max(max_wallclock_ms - elapsed_ms, 0.0)
        if remaining_ms > warmdown_ms:
            return 1.0  # Stable phase: full LR
        # Linear decay to 0 (sharp)
