...
def quantize_int5_per_row(t: Tensor, clip_range: int = 15) -> tuple[Tensor, Tensor]:
    ...
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    return q, scale
def _estimate_compressed_size(q: Tensor, s: Tensor) -> int:
    """Estimate zstd-compressed size of a quantized tensor pair."""
    buf = io.BytesIO()
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    # Phase 1: Compute per-tensor cost and benefit for knapsack optimization
    tensor_info = []  # list of (name, saving_bytes, sensitivity_cost, q5, s5)
    int6_cache = {}   # name -> (q6, s6) for reuse in Phase 3
    for name, tensor in state_dict.items():
        ...
        int6_cache[name] = (q6, s6)
        q5, s5 = quantize_int5_per_row(t)
        # Measure compressed size difference
        size6 = _estimate_compressed_size(q6, s6)
        size5 = _estimate_compressed_size(q5, s5)
        saving = size6 - size5  # positive = int5 is smaller
        if saving <= 0:
            continue  # skip tensors where int5 doesn't help
        # Measure sensitivity
        recon6 = q6.float() * s6.float().view(q6.shape[0], *([1] * (q6.ndim - 1)))
        recon5 = q5.float() * s5.float().view(q5.shape[0], *([1] * (q5.ndim - 1)))
        t32 = t.float()
        err6 = (t32 - recon6).pow(2).mean().item()
        err5 = (t32 - recon5).pow(2).mean().item()
        cost = max(err5 - err6, 1e-12)  # avoid division by zero
        tensor_info.append((name, saving, cost, q5, s5))

    # Phase 2: Greedy knapsack — sort by cost/saving ratio (ascending = best deals first)
    tensor_info.sort(key=lambda x: x[2] / x[1])

    # Target: need to save enough to get under 16MB
    TARGET_SAVING = 2_200_000  # bytes
    cumulative_saving = 0
    int5_tensors = {}  # name -> (q5, s5) cached results
    for name, saving, cost, q5, s5 in tensor_info:
        if cumulative_saving >= TARGET_SAVING:
            break
        int5_tensors[name] = (q5, s5)
        cumulative_saving += saving

    print(f"Knapsack-EWQ: {len(int5_tensors)} tensors -> int5, "
          f"estimated saving: {cumulative_saving} bytes, target: {TARGET_SAVING} bytes")

    # Phase 3: Quantize using the selected assignment (with cached results)
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            if name in int5_tensors:
                q, s = int5_tensors[name]
                result[name + ".q"] = q
                result[name + ".scale"] = s
                meta[name] = {"type": "int5"}
            else:
                # Use cached int6 result if available
                if name in int6_cache:
                    q, s = int6_cache[name]
                else:
