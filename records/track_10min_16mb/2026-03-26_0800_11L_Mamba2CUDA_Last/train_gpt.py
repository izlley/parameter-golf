...
# Ensure CUDA runtime is discoverable for mamba_ssm / causal_conv1d
_cuda_lib = "/opt/conda/envs/cuda128/lib"
if os.path.isdir(_cuda_lib):
    os.environ.setdefault("LD_LIBRARY_PATH", "")
    if _cuda_lib not in os.environ["LD_LIBRARY_PATH"]:
        os.environ["LD_LIBRARY_PATH"] = _cuda_lib + ":" + os.environ["LD_LIBRARY_PATH"]
...
try:
    ...
    _USE_FA3 = False
# CUDA-accelerated Mamba2 SSM
try:
    from mamba_ssm.modules.mamba2 import Mamba2 as _Mamba2CUDA
    _USE_MAMBA_CUDA = True
except ImportError:
    _USE_MAMBA_CUDA = False
...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.proj is not None:
            h = self.proj(h)
        return h * self.scale.to(dtype=h.dtype)
# ===== Mamba2 CUDA-accelerated (mamba_ssm package) =====
class Mamba2Layer(nn.Module):
    """CUDA-accelerated Mamba-2 layer using mamba_ssm Triton kernels.
    Drop-in replacement for CausalSelfAttention.
    Input: (batch, seqlen, dim), Output: (batch, seqlen, dim)."""
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4,
                 expand: int = 1, chunk_size: int = 256):
        super().__init__()
        assert _USE_MAMBA_CUDA, "mamba_ssm package required but not found"
        self.ssm = _Mamba2CUDA(
            d_model=d_model,
            d_state=d_state,
            d_conv=d_conv,
            expand=expand,
        ...
        with torch.no_grad():
            self.ssm.out_proj.weight.zero_()
...
class Mamba2Block(nn.Module):
    """Block that uses CUDA-accelerated Mamba2 instead of attention, keeping MLP."""
    def __init__(self, dim: int, mlp_mult: int, d_state: int = 16, d_conv: int = 4,
                 expand: int = 1, chunk_size: int = 256, layer_idx: int = 0,
                 ln_scale: bool = False):
        super().__init__()
...
def main() -> None:
    ...
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
    # Keep mamba_ssm Mamba2 params in float32 for optimizer precision (same as CastedLinear)
    if _USE_MAMBA_CUDA:
        for module in base_model.modules():
            if isinstance(module, _Mamba2CUDA):
                module.float()
