...
class LoRAAdapter(nn.Module):
    """Low-rank adapter: delta_W = A @ B, applied as output += LoRA(input)."""
...
class GPT(nn.Module):
    def __init__(
        ...
        # Relaxed Recursive: share layers 4->5, 6->7 with LoRA adapters
        shared_pairs = [(4, 5), (6, 7)]  # (source, target) pairs
        shared_targets = {t: s for s, t in shared_pairs}
        blocks: list[nn.Module] = []
        for i in range(num_layers):
            if i in shared_targets:
                # Reuse source block (same nn.Module object)
                src_idx = shared_targets[i]
                blocks.append(blocks[src_idx])
            else:
                blocks.append(Block(
        ...
        # LoRA adapters for shared target layers (stored separately since blocks are shared)
        self.lora_adapters = nn.ModuleDict()
        for src, tgt in shared_pairs:
            self.lora_adapters[f"lora_attn_{tgt}"] = LoRAAdapter(model_dim, model_dim, rank=4)
            self.lora_adapters[f"lora_mlp_{tgt}"] = LoRAAdapter(model_dim, model_dim, rank=4)
        # Pre-compute boolean list for torch.compile compatibility
        self._is_shared_target: list[bool] = [i in shared_targets for i in range(num_layers)]
        if rope_dims > 0:
    ...
    def _apply_lora(self, x: Tensor, layer_idx: int) -> Tensor:
        """Apply LoRA correction for shared target layers."""
        normed = F.rms_norm(x, (x.size(-1),))
        x = x + self.lora_adapters[f"lora_attn_{layer_idx}"](normed)
        x = x + self.lora_adapters[f"lora_mlp_{layer_idx}"](normed)
        return x
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if self._is_shared_target[i]:
                x = self._apply_lora(x, i)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if self._is_shared_target[bi]:
                x = self._apply_lora(x, bi)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    # LoRA adapter params (small rank-4 matrices) -> AdamW scalar optimizer
    for lora_p in base_model.lora_adapters.parameters():
        scalar_params.append(lora_p)
