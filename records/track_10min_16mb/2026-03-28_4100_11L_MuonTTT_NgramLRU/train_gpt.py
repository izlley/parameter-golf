...
class Hyperparameters:
    ...
    ngram_blend_alpha = float(os.environ.get("NGRAM_BLEND_ALPHA", 0.05))  # blend weight for ngram
    ngram_max_order = int(os.environ.get("NGRAM_MAX_ORDER", 5))  # max n-gram order (context length)
    ngram_min_count = int(os.environ.get("NGRAM_MIN_COUNT", 2))  # minimum count to use n-gram prediction
...
class NgramCache:
    ...
    def update_batch(self, token_ids: list[int]) -> None:
        V = self.vocab_size
        # LRU: if adding tokens exceeds limit, rebuild from recent
        ...
        for order in range(1, max_order + 1):
            cdict = counts[order - 1]
            for i in range(n - order):
                ctx = tuple(token_ids[i:i + order])
                tgt = token_ids[i + order]
                if ctx in cdict:
                    cdict[ctx][tgt] += 1
                else:
                    arr = [0] * V
    ...
    def get_log_probs_batch(self, x_seq: torch.Tensor, start_pos: int, end_pos: int,
        ...
        if self.unigram_total < self.min_count:
            return None
        V = self.vocab_size
        uni_lp = torch.tensor(self.unigram, dtype=torch.float32, device=device)
        uni_lp = ((uni_lp + 1.0) / (self.unigram_total + V)).log()
...
def eval_val_sliding_ttt(
    ...
    log0(f"supermask_ttt:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"ngram_alpha={args.ngram_blend_alpha} ngram_order={args.ngram_max_order}")
    ...
    # N-gram backoff cache with LRU limit
