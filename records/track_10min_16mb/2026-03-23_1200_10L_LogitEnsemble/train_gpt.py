...
def eval_val_sliding_ensemble(
    args: Hyperparameters,
    base_model: nn.Module,
    swa_sd: dict[str, Tensor],
    last_sd: dict[str, Tensor],
    rank: int,
    world_size: int,
    device: torch.device,
    val_tokens: Tensor,
    ...
    with torch.no_grad():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            # Forward with SWA model
            for name, param in base_model.named_parameters():
                param.data.copy_(swa_sd[name])
            for name, buf in base_model.named_buffers():
                if name in swa_sd:
                    buf.data.copy_(swa_sd[name])
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits1 = base_model.forward_logits(x_batch)

            # Forward with last checkpoint
            for name, param in base_model.named_parameters():
                param.data.copy_(last_sd[name])
            for name, buf in base_model.named_buffers():
                if name in last_sd:
                    buf.data.copy_(last_sd[name])
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits2 = base_model.forward_logits(x_batch)
...
def main() -> None:
    ...
    # Save last training checkpoint for ensemble
    last_sd = {k: v.detach().cpu().clone() for k, v in base_model.state_dict().items()}
    ...
    # Ensemble eval: SWA model + last checkpoint
    if last_sd is not None:
        log0("ensemble_eval:start")
        # Quantize last checkpoint too
        last_sd_pruned = {}
        for name, param in last_sd.items():
            p = param.clone()
            if p.ndim == 2 and p.numel() > 65536:
                threshold = torch.quantile(p.abs().float().flatten(), 0.05)
                mask = p.abs() < threshold
                p.masked_fill_(mask, 0.0)
            last_sd_pruned[name] = p
        last_quant, last_meta = mixed_quantize_int6(last_sd_pruned, {"mlp", "attn", "bigram"})
        last_deq = dequantize_mixed_int6(last_quant, last_meta, last_sd_pruned)

        swa_sd_device = {k: v.to(device) for k, v in base_model.state_dict().items()}
        last_sd_device = {k: v.to(device) for k, v in last_deq.items()}

        # Ensemble sliding eval
        e_val_loss, e_val_bpb = eval_val_sliding_ensemble(
            args, base_model, swa_sd_device, last_sd_device,
            rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, batch_seqs=args.eval_batch_seqs,
        )
        log0(f"ensemble_val_loss:{e_val_loss:.4f} ensemble_val_bpb:{e_val_bpb:.6f}")

    if master_process and _HAS_WANDB:
        log_dict = {"final_val_loss": q_val_loss, "final_val_bpb": q_val_bpb,
                     "quant_file_bytes": quant_file_bytes, "total_bytes": total_bytes}
        if last_sd is not None:
            log_dict["ensemble_val_loss"] = e_val_loss
            log_dict["ensemble_val_bpb"] = e_val_bpb
        wandb.log(log_dict)
        wandb.finish()
