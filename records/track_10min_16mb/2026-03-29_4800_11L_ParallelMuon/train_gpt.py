...
class ParallelMuon(torch.optim.Optimizer):
    """Muon with batched Newton-Schulz for 2D params of compatible shapes.

    Groups all 2D matrix parameters by their (rows, cols) shape and processes
    each group as a single batched tensor via torch.bmm, dramatically reducing
    optimizer time compared to sequential per-parameter Newton-Schulz iterations.
    """
    def __init__(self, params, lr: float, momentum: float, backend_steps: int,
                 nesterov: bool = True, weight_decay: float = 0.0):
        super().__init__(
    ...
    @staticmethod
    def _batched_newton_schulz(G_batch: Tensor, steps: int, eps: float = 1e-7) -> Tensor:
        """Apply Newton-Schulz iteration to a batch of matrices simultaneously.

        Args:
            G_batch: (batch, rows, cols) tensor of gradient matrices
            steps: number of Newton-Schulz iterations
            eps: epsilon for numerical stability
        Returns:
            (batch, rows, cols) tensor of orthogonalized matrices
        """
        a, b, c = (3.4445, -4.7750, 2.0315)
        X = G_batch.bfloat16()
        # Normalize each matrix in the batch independently
        norms = X.flatten(1).norm(dim=1).unsqueeze(-1).unsqueeze(-1) + eps
        X = X / norms
        # Transpose if tall matrices (rows > cols) for numerical stability
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            nesterov = group["nesterov"]
            wd = group.get("weight_decay", 0.0)
            # Step 1: Apply momentum to all grads and collect post-momentum grads
            ...
            for i, p in enumerate(params):
                ...
                if nesterov:
                    g = g.add(buf, alpha=momentum)
                grads[i] = g
            # Step 2: Group params by shape for batched Newton-Schulz
            # shape_key -> list of (param_index, grad_tensor)
            shape_groups: dict[tuple[int, int], list[tuple[int, Tensor]]] = {}
            for i, g in enumerate(grads):
                if g is not None:
                    key = (g.size(0), g.size(1))
                    if key not in shape_groups:
                        shape_groups[key] = []
                    shape_groups[key].append((i, g))
            # Step 3: Batched Newton-Schulz + NorMuon normalize per shape group
            # Store processed updates keyed by param index
            updates: dict[int, Tensor] = {}
            for (rows, cols), group_items in shape_groups.items():
                indices = [item[0] for item in group_items]
                grad_list = [item[1] for item in group_items]
                # Stack into (batch, rows, cols) for batched processing
                G_batch = torch.stack(grad_list, dim=0)
                # Batched Newton-Schulz
                X_batch = self._batched_newton_schulz(G_batch, steps=backend_steps)
                # NorMuon: F.normalize along dim=1 (row-wise) for each matrix
                # X_batch shape: (batch, rows, cols)
                X_batch = F.normalize(X_batch, dim=2)  # dim=2 corresponds to dim=1 per matrix
                # Scale factor
                scale = max(1, rows / cols) ** 0.5
                X_batch = X_batch * scale
                # Scatter back to individual updates
                for j, idx in enumerate(indices):
                    updates[idx] = X_batch[j]
            # Step 4: Build flat update buffer and all_reduce
            total_params = sum(int(p.numel()) for p in params)
            updates_flat = torch.zeros(total_params, device=params[0].device, dtype=torch.bfloat16)
            curr = 0
            for i, p in enumerate(params):
            ...
            if distributed:
                dist.all_reduce(updates_flat, op=dist.ReduceOp.SUM)
            # Step 5: Apply updates with weight decay
            curr = 0
            for p in params:
                if wd > 0.0:
                    p.data.mul_(1.0 - lr * wd)
