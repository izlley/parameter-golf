...
class Hyperparameters:
    ...
    eminf_steps = int(os.environ.get("EMINF_STEPS", 5))
    eminf_lr = float(os.environ.get("EMINF_LR", 0.05))
...
def eval_val_sliding_eminf(
    ...
    """Sliding window evaluation with EM-INF logit refinement."""
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= 1]
    ...
    for bi in range(0, len(my_windows), batch_seqs):
        ...
        with torch.inference_mode():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = compiled_logits(x_batch)
        # EM-INF: refine logits outside inference_mode
        logits_refined = logits.detach().float().clone().requires_grad_(True)
        opt = torch.optim.SGD([logits_refined], lr=eminf_lr)
        for _ in range(eminf_steps):
            opt.zero_grad()
            probs = F.softmax(logits_refined, dim=-1)
            ent = -(probs * (probs + 1e-10).log()).sum(dim=-1).mean()
            ent.backward()
            opt.step()
        logits_final = logits_refined.detach()
        nll = F.cross_entropy(
            logits_final.reshape(-1, logits_final.size(-1)),
            y_batch.reshape(-1),
            reduction="none",
        ).reshape(bsz, seq_len)
        for i, ws in enumerate(batch_ws):
...
def main() -> None:
    ...
    # EM-INF: entropy-minimized sliding window evaluation (post-TTT)
    if master_process:
        log0("eminf:starting entropy minimization evaluation (post-TTT)")
    torch.cuda.synchronize()
    t_eminf = time.perf_counter()
    eminf_loss, eminf_bpb = eval_val_sliding_eminf(
        args, eval_model, rank, world_size, device,
        val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        stride=args.eval_stride,
        eval_seq_len=sw_seq_len,
        eminf_steps=args.eminf_steps, eminf_lr=args.eminf_lr,
        log0=log0,
    )
    torch.cuda.synchronize()
    log0(f"eminf val_loss:{eminf_loss:.4f} val_bpb:{eminf_bpb:.4f} "
         f"eval_time:{1000.0 * (time.perf_counter() - t_eminf):.0f}ms")
    log0(f"eminf_exact val_loss:{eminf_loss:.8f} val_bpb:{eminf_bpb:.8f}")
    if _HAS_WANDB and master_process:
        wandb.log({"eminf_loss": eminf_loss, "eminf_bpb": eminf_bpb})
    if distributed:
