...
def eval_val_sliding_ttt(
    ...
    for block in base_model.blocks:
        ...
        # MLP mask: per hidden channel (init=5.0 → sigmoid≈0.993)
        mlp_ms = torch.full((hidden_dim,), 5.0, device=device, requires_grad=True)
        # MLP bias: additive shift in hidden space (init=0)
        mlp_bias = torch.zeros(hidden_dim, device=device, requires_grad=True)
        # Attn mask: per model_dim channel (init=5.0 → sigmoid≈0.993)
        attn_ms = torch.full((attn_dim,), 5.0, device=device, requires_grad=True)
