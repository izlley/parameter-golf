...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def gptq_quantize_int6(W: Tensor, H_diag: Tensor | None = None, clip_range: int = 31, group_size: int = 8) -> tuple[Tensor, Tensor]:
    """GPTQ with diagonal Hessian approximation for int6 quantization.
    W: (out_features, in_features) weight matrix
    H_diag: (in_features,) diagonal of Hessian (X^T X / n_samples), or None for identity
    Returns (Q_int8, scale_fp16) where Q values are in [-31, 31].
    """
    W = W.float().clone()
    n_rows, n_cols = W.shape
    # Find best per-row scale via percentile search (same as quantize_int6_per_row)
    best_scale = None
    best_err = float('inf')
    for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
        if pct < 1.0:
        ...
        s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
        q_test = torch.clamp(torch.round(W / s.float()[:, None]), -clip_range, clip_range)
        recon = q_test * s.float()[:, None]
    ...
    scale = best_scale
    scale_f = scale.float()
    Q = torch.zeros(n_rows, n_cols, dtype=torch.int8)
    # If no Hessian provided, fall back to simple rounding (equivalent to quantize_int6_per_row)
    if H_diag is None:
        Q = torch.clamp(torch.round(W / scale_f[:, None]), -clip_range, clip_range).to(torch.int8)
        return Q, scale
    # Ensure H_diag is float and clamped
    H_diag = H_diag.float().clamp_min(1e-12)
    # Process columns in groups for GPTQ error compensation
    for col_start in range(0, n_cols, group_size):
        ...
        for j in range(col_start, col_end):
            ...
            err = w_col - q_col * scale_f  # (n_rows,)
            # Distribute error to remaining columns within the group
            remaining_end = col_end
            if j + 1 < remaining_end:
                h_jj = H_diag[j]
                remaining_h = H_diag[j + 1:remaining_end]  # (remaining,)
                # correction weights: how much each remaining col should absorb
                correction_weights = (remaining_h / h_jj).unsqueeze(0)  # (1, remaining)
                W[:, j + 1:remaining_end] += err.unsqueeze(1) * correction_weights
    return Q, scale
def collect_gptq_hessian(
    ...
    """Collect diagonal Hessian (X^T X / n_samples) for each Linear layer.
    Hooks into all CastedLinear (and nn.Linear) layers, runs a few val batches,
    and returns per-layer Hessian diagonals keyed by parameter name (weight name).
    """
    hessian_dict: dict[str, Tensor] = {}
    sample_counts: dict[str, int] = {}
    hooks = []
    # Map module to its weight parameter name in state_dict
    name_map: dict[nn.Module, str] = {}
    for name, mod in model.named_modules():
        if isinstance(mod, (CastedLinear, nn.Linear)) and hasattr(mod, 'weight') and mod.weight.ndim == 2:
            weight_name = name + ".weight"
            name_map[mod] = weight_name
            in_features = mod.weight.shape[1]
            hessian_dict[weight_name] = torch.zeros(in_features, dtype=torch.float64)
            sample_counts[weight_name] = 0
    def make_hook(module: nn.Module):
        def hook_fn(mod, inp, out):
            wname = name_map.get(mod)
            if wname is None:
                return
            x = inp[0]  # (batch, seq, in_features) or (batch*seq, in_features)
            if x.ndim == 3:
                x = x.reshape(-1, x.shape[-1])  # flatten to 2D
            x = x.float()
            # Accumulate X^T X diagonal = sum of x_i^2 per feature
            hessian_dict[wname] += x.pow(2).sum(dim=0).cpu().to(torch.float64)
            sample_counts[wname] += x.shape[0]
        return hook_fn
    for mod in name_map:
    ...
    for h in hooks:
        h.remove()
    # Normalize: H_diag = X^T X / n_samples
    for wname in hessian_dict:
        n = sample_counts.get(wname, 1)
        if n > 0:
            hessian_dict[wname] = (hessian_dict[wname] / n).float()
        else:
            hessian_dict[wname] = hessian_dict[wname].float()
    return hessian_dict
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessian_dict: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            h_diag = hessian_dict.get(name) if hessian_dict is not None else None
            if h_diag is not None and t.ndim == 2:
                q, s = gptq_quantize_int6(t, h_diag)
                gptq_count += 1
            else:
...
def main() -> None:
    ...
    # GPTQ calibration: collect Hessian diagonals from validation data
    t_gptq_cal = time.perf_counter()
    log0("gptq_calibration:start collecting Hessian diagonals from val data")
    hessian_dict = collect_gptq_hessian(
        base_model, val_tokens, device,
        seq_len=args.train_seq_len, n_batches=8, batch_seqs=4,
    )
    log0(f"gptq_calibration:done layers={len(hessian_dict)} elapsed={time.perf_counter() - t_gptq_cal:.1f}s")
    quant_result, quant_meta, gptq_count = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, hessian_dict=hessian_dict)
    log0(f"gptq_quantize:done gptq_layers={gptq_count}")
