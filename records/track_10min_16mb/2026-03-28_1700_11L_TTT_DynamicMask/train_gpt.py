...
class DynamicMaskGenerator(nn.Module):
    """Lightweight input-dependent mask generator."""
    def __init__(self, input_dim: int, output_dim: int, bottleneck: int = 64):
        super().__init__()
        self.down = nn.Linear(input_dim, bottleneck, bias=False)
        self.up = nn.Linear(bottleneck, output_dim, bias=True)
        # Initialize to output ~3.0 (sigmoid ≈ 0.953) like static init
        nn.init.zeros_(self.down.weight)
        nn.init.zeros_(self.up.weight)
        nn.init.constant_(self.up.bias, 3.0)
    def forward(self, x: Tensor) -> Tensor:
        # x: (batch, seq_len, dim) → pool to (batch, dim) → mask (batch, output_dim)
        h = x.mean(dim=1)  # (batch, input_dim)
        return self.up(F.relu(self.down(h.float()))).to(x.dtype)  # (batch, output_dim)
def eval_val_sliding_ttt(
    ...
    # Create dynamic mask generators + bias terms for each block
    mask_generators: list[nn.Module] = []
    mask_scores: list[Tensor] = []  # keep for optimizer — will be populated from generator params
    ...
    for block in base_model.blocks:
        ...
        model_dim = block.attn.proj.weight.shape[1] if block.attn.proj.weight.ndim == 2 else attn_dim
        # Dynamic generators
        mlp_gen = DynamicMaskGenerator(model_dim, hidden_dim, bottleneck=64).to(device)
        attn_gen = DynamicMaskGenerator(model_dim, attn_dim, bottleneck=64).to(device)
        # Bias terms (still static)
        ...
        mask_generators.extend([mlp_gen, attn_gen])
        mask_scores.extend([mlp_bias, attn_bias])  # static params
    ...
    all_ttt_params = list(mask_scores)  # biases
    for gen in mask_generators:
        all_ttt_params.extend(gen.parameters())

    # Monkey-patch forwards with dynamic masked+biased versions
    for bi, block in enumerate(base_model.blocks):
        mlp_gen = mask_generators[bi * 2]
        attn_gen = mask_generators[bi * 2 + 1]
        mlp_bias = mask_scores[bi * 2]
        attn_bias = mask_scores[bi * 2 + 1]
        fc_layer = block.mlp.fc
        proj_layer = block.mlp.proj
        def _make_masked_mlp_fwd(fc, proj, gen, bias):
            def masked_forward(x):
                h = F.leaky_relu(fc(x), negative_slope=0.5)
                h = h.square()
                mask = torch.sigmoid(gen(x)).unsqueeze(1)  # (batch, 1, hidden_dim)
                h = h * mask + bias.to(h.dtype)
                return proj(h)
            return masked_forward
        block.mlp.forward = _make_masked_mlp_fwd(fc_layer, proj_layer, mlp_gen, mlp_bias)
        # Monkey-patch Attn forward: apply dynamic channel mask + bias to output
        orig_attn_fwd = block.attn.forward.__func__ if hasattr(block.attn.forward, '__func__') else None
        def _make_masked_attn_fwd(attn_module, gen, bias, orig_fwd):
            def masked_attn_forward(x, v_embed=None):
                out = orig_fwd(attn_module, x, v_embed=v_embed)
                mask = torch.sigmoid(gen(x)).unsqueeze(1)  # (batch, 1, attn_dim)
                return out * mask + bias.to(out.dtype)
            return masked_attn_forward
        if orig_attn_fwd is not None:
            block.attn.forward = _make_masked_attn_fwd(block.attn, attn_gen, attn_bias, orig_attn_fwd)

    total_mask_params = sum(p.numel() for p in all_ttt_params)
    ...
    optimizer = torch.optim.SGD(all_ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        torch.nn.utils.clip_grad_norm_(all_ttt_params, args.ttt_grad_clip)
                        optimizer.step()
