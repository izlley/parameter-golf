...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.proj is not None:
            h = self.proj(h)
        return h * self.scale.to(dtype=h.dtype)
# ===== Gated Linear Attention (Pure PyTorch, torch.compile(fullgraph=True) compatible) =====
def _gla_chunk_fwd(q: Tensor, k: Tensor, v: Tensor, g: Tensor, chunk_size: int) -> Tensor:
...
class GLALayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int = 16, chunk_size: int = 128):
        ...
        self.v_proj = CastedLinear(d_model, d_model, bias=False)
        # Gate projection: per-head scalar gate
        self.gate_proj = CastedLinear(d_model, num_heads, bias=True)
        nn.init.zeros_(self.gate_proj.weight)
        nn.init.constant_(self.gate_proj.bias, 4.0)  # Start with high gate (= slow decay)
        # Output
...
class Mamba2Block(nn.Module):
    """Block with GLA (Gated Linear Attention) + optionally shared MLP."""
    def __init__(self, dim: int, mlp_mult: int, d_state: int = 32, d_conv: int = 2,
        ...
        # Use GLA instead of Mamba2
        num_heads = (expand * dim) // 64  # Match Mamba2 head count
        self.ssm = GLALayer(dim, num_heads=num_heads, chunk_size=chunk_size)
