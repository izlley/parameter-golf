...
class Hyperparameters:
    ...
    swa_start_frac = float(os.environ.get("SWA_START_FRAC", 0.50))
    swa_every = int(os.environ.get("SWA_EVERY", 50))

    qat_enabled = bool(int(os.environ.get("QAT_ENABLED", "1")))

    # Multi-Exit: auxiliary prediction heads at intermediate decoder layers
    exit_decoder_indices: str = os.environ.get("EXIT_DECODER_INDICES", "1,3")  # decoder layer indices (0-based)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Multi-Exit: auxiliary norms at intermediate decoder layers
        self.exit_decoder_indices = exit_decoder_indices or []
        self.aux_norms = nn.ModuleDict()
        for idx in self.exit_decoder_indices:
            self.aux_norms[str(idx)] = RMSNorm()
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, aux_loss_weight: float = 0.1) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            # Multi-Exit: compute auxiliary loss at intermediate decoder layers
            if i in self.exit_decoder_indices:
                h_aux = self.aux_norms[str(i)](x).reshape(-1, x.size(-1))
                aux_logits = self._project_logits(h_aux)
                aux_losses.append(F.cross_entropy(aux_logits.float(), targets, reduction="mean"))
        x = self.final_norm(x).reshape(-1, x.size(-1))
        main_logits = self._project_logits(x)
        main_loss = F.cross_entropy(main_logits.float(), targets, reduction="mean")
        if aux_losses:
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[self.num_encoder_layers + i](x, x0)
            # Multi-Exit: collect logits from intermediate exits
            if i in self.exit_decoder_indices:
                h_aux = self.aux_norms[str(i)](x)
                all_logits.append(self._project_logits(h_aux))
        # Final exit
        main_logits = self._project_logits(self.final_norm(x))
        all_logits.append(main_logits)
        # Average all exit logits (ensemble)
...
def main() -> None:
    ...
    # Parse exit decoder indices
    exit_decoder_indices = [int(x) for x in args.exit_decoder_indices.split(",") if x.strip()]
    log0(f"multi_exit:decoder_indices={exit_decoder_indices} aux_loss_weight={args.aux_loss_weight}")
