...
def pack_int6_to_bytes(q: Tensor) -> bytes:
    """Pack int6 values (-31..31) into 6 bits each. 4 values = 3 bytes."""
    flat = q.reshape(-1).to(torch.int8).numpy().astype(np.int8)
    unsigned = (flat.astype(np.int16) + 31).astype(np.uint8)
    n = len(unsigned)
    pad = (4 - n % 4) % 4
    if pad:
        unsigned = np.concatenate([unsigned, np.zeros(pad, dtype=np.uint8)])
    a, b, c, d = unsigned[0::4], unsigned[1::4], unsigned[2::4], unsigned[3::4]
    b0 = (a << 2) | (b >> 4)
    b1 = ((b & 0x0F) << 4) | (c >> 2)
    b2 = ((c & 0x03) << 6) | d
    packed = np.stack([b0, b1, b2], axis=1).reshape(-1).tobytes()
    return packed

def unpack_int6_from_bytes(data: bytes, n: int) -> Tensor:
    """Unpack 6-bit packed bytes back to int6 tensor."""
    ...
    c = ((b1 & 0x0F) << 2) | (b2 >> 6)
    d = b2 & 0x3F
    unsigned = np.stack([a, b, c, d], axis=1).reshape(-1)[:n]
    signed = unsigned.astype(np.int16) - 31
    return torch.from_numpy(signed.astype(np.int8))

def serialize_raw_binary(quant_result: dict, quant_meta: dict) -> bytes:
    """Serialize quantized model to raw binary format (no torch.save overhead)."""
    buf = io.BytesIO()
    meta_bytes = json.dumps(quant_meta).encode('utf-8')
    buf.write(struct.pack('<I', len(meta_bytes)))
    buf.write(meta_bytes)
    sorted_names = sorted(quant_result.keys())
    buf.write(struct.pack('<I', len(sorted_names)))
    for name in sorted_names:
        ...
        shape = list(t.shape)
        base_name = name[:-2] if name.endswith('.q') else None
        is_int6_q = (base_name is not None and
                     isinstance(quant_meta.get(base_name), dict) and
                     quant_meta[base_name].get('type') == 'int6')
        if is_int6_q:
            dtype_code = 3
            ...
            for s in shape:
                buf.write(struct.pack('<I', s))
            raw_bytes = t.contiguous().numpy().tobytes()
            buf.write(struct.pack('<I', len(raw_bytes)))
            buf.write(raw_bytes)
    return buf.getvalue()

def deserialize_raw_binary(data: bytes) -> tuple[dict, dict]:
    """Deserialize raw binary format back to quant_result and quant_meta."""
    buf = io.BytesIO(data)
    dtype_map = {0: (torch.int8, np.int8), 1: (torch.float16, np.float16),
                 2: (torch.float32, np.float32), 4: (torch.int32, np.int32), 5: (torch.int64, np.int64)}
    meta_len = struct.unpack('<I', buf.read(4))[0]
    quant_meta = json.loads(buf.read(meta_len).decode('utf-8'))
    ...
    for _ in range(num_tensors):
        ...
        if dtype_code == 3:
            ...
            for s in shape:
                numel *= s
            t = unpack_int6_from_bytes(raw, numel).reshape(shape)
        else:
            torch_dtype, np_dtype = dtype_map[dtype_code]
...
def main() -> None:
    ...
    quant_raw = serialize_raw_binary(quant_result, quant_meta)
    ...
    quant_raw_disk = zstandard.ZstdDecompressor().decompress(quant_blob_disk) if _COMPRESSOR == "zstd" else zlib.decompress(quant_blob_disk)
    quant_result_loaded, quant_meta_loaded = deserialize_raw_binary(quant_raw_disk)
