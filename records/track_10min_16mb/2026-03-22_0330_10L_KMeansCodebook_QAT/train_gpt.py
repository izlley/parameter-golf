...
def kmeans_quantize_per_row(t: Tensor, num_centroids: int = 32,
                            num_iters: int = 20, subsample: int = 4096) -> tuple[Tensor, Tensor, Tensor]:
    """K-means codebook quantization with per-row scaling.

    Returns: (indices [int8], per_row_scale [fp16], codebook [fp16, num_centroids])
    """
    t32 = t.float()
    if t32.ndim != 2:
        t32 = t32.view(1, -1)

    # Per-row normalization (same as uniform quant)
    row_max = t32.abs().amax(dim=1)
    scale = row_max.clamp_min(1e-12).to(torch.float16)
    scale = scale.clamp_min(torch.finfo(torch.float16).tiny)
    t_norm = t32 / scale.float()[:, None]  # normalized to ~[-1, 1]

    # Flatten all normalized values for k-means
    flat = t_norm.reshape(-1)

    # Subsample for faster k-means
    if flat.numel() > subsample:
        idx = torch.randperm(flat.numel())[:subsample]
        samples = flat[idx]
    else:
        samples = flat

    # Initialize centroids with uniform spacing (linspace within data range)
    vmin, vmax = samples.min().item(), samples.max().item()
    centroids = torch.linspace(vmin, vmax, num_centroids, dtype=torch.float32)

    # K-means iterations
    for _ in range(num_iters):
        # Assign samples to nearest centroid
        dists = (samples[:, None] - centroids[None, :]).abs()
        assignments = dists.argmin(dim=1)
        # Update centroids
        for c in range(num_centroids):
            mask = assignments == c
            if mask.any():
                centroids[c] = samples[mask].mean()

    # Sort centroids for consistent ordering
    centroids, sort_idx = centroids.sort()

    # Assign all values to nearest centroid
    dists = (flat[:, None] - centroids[None, :]).abs()
    indices = dists.argmin(dim=1).to(torch.int8)
    indices = indices.view(t_norm.shape)

    return indices, scale, centroids.to(torch.float16)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            num_c = 32 if cat == "mlp" else 64  # match int5/int6 level counts
            indices, scale, codebook = kmeans_quantize_per_row(t, num_centroids=num_c)
            result[name + ".q"] = indices
            result[name + ".scale"] = scale
            result[name + ".codebook"] = codebook
            meta[name] = {"type": f"kmeans{num_c}"}
        else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info["type"].startswith("kmeans"):
            # Codebook-based dequantization
            codebook = result[name + ".codebook"].float()
            q_long = q.long().clamp(0, codebook.numel() - 1)
            w_norm = codebook[q_long]  # lookup normalized values
            out[name] = (w_norm * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))).to(orig_dtype)
        elif s.ndim > 0:
