...
class Hyperparameters:
    ...
    distill_threshold = float(os.environ.get("DISTILL_THRESHOLD", 0.5))  # activate when lr_scale < this
    distill_sync_every = int(os.environ.get("DISTILL_SYNC_EVERY", 100))  # sync teacher weights every N steps
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> tuple[Tensor, Tensor]:
        ...
        # Always return (loss, logits) for Self-Distillation compatibility
...
def main() -> None:
    ...
    # Self-Distillation: create teacher model (EMA copy, no grad, no compile)
    teacher_model = GPT(
    ...
    while True:
        ...
        # Self-Distillation: activate when lr_scale < threshold
        if not distill_active and scale < args.distill_threshold:
            distill_active = True
            # Sync EMA weights to teacher for the first time
            teacher_sd = {name: t.to(dtype=teacher_model.state_dict()[name].dtype) for name, t in ema_state.items() if name in teacher_model.state_dict()}
            teacher_model.load_state_dict(teacher_sd, strict=True)
            log0(f"self_distill:enabled step:{step} scale:{scale:.4f} alpha:{args.distill_alpha} T:{args.distill_temperature}")
        elif distill_active and step % args.distill_sync_every == 0:
            # Periodically sync EMA weights to teacher
            teacher_sd = {name: t.to(dtype=teacher_model.state_dict()[name].dtype) for name, t in ema_state.items() if name in teacher_model.state_dict()}
            teacher_model.load_state_dict(teacher_sd, strict=True)
        zero_grad_all()
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ce_loss, student_logits = model(x, y)
                if distill_active:
                    with torch.no_grad():
                        teacher_logits = teacher_model.forward_logits(x)  # (B, T, V)
                    T = args.distill_temperature
                    # student_logits is (B*T, V), teacher_logits is (B, T, V)
                    ...
                    loss = ce_loss + args.distill_alpha * kl_loss
                else:
                    loss = ce_loss
    ...
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        if _HAS_WANDB and master_process:
            wandb.log({"final_int6_s64_loss": sw64_val_loss, "final_int6_s64_bpb": sw64_val_bpb})
    if distributed:
