...
class Hyperparameters:
    ...
    sparsity_lambda = float(os.environ.get("SPARSITY_LAMBDA", "0.0001"))
...
def main() -> None:
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            # Compression-friendly L1 regularization on large weight matrices
            if args.sparsity_lambda > 0:
                l1_pen = sum(p.float().abs().mean() for p in base_model.parameters() if p.ndim == 2 and p.numel() > 65536)
                loss = loss + (args.sparsity_lambda / grad_accum_steps) * l1_pen
