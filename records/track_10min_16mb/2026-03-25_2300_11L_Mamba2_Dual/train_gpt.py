...
class Hyperparameters:
    ...
    mamba_layers = os.environ.get("MAMBA_LAYERS", "0,10")
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,ssm_scale,dt_bias,A_log,.D",
    ).split(",")
    if pattern
)
...
def main() -> None:
    ...
    log0(f"hybrid_arch:mamba_layers={mamba_indices} attn_layers={[i for i in range(args.num_layers) if i not in base_model.mamba_layer_set]}")
