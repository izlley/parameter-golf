...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            meta[name] = {"type": "int8"}
    return result, meta
def delta_encode_tensors(quant_result: dict[str, Tensor]) -> tuple[dict[str, Tensor], dict[str, tuple]]:
    """Apply delta encoding to quantized int8 .q tensors for better zstd compression.
    Int6 values are in [-31, 31], so deltas fit in [-62, 62] ⊂ int8 range."""
    encoded: dict[str, Tensor] = {}
    shapes: dict[str, tuple] = {}
    for name, tensor in quant_result.items():
        if name.endswith(".q") and tensor.dtype == torch.int8:
            shapes[name] = tuple(tensor.shape)
            flat = tensor.flatten().to(torch.int16)
            delta = torch.empty_like(flat)
            delta[0] = flat[0]
            delta[1:] = flat[1:] - flat[:-1]
            encoded[name] = delta.to(torch.int8)
        else:
            encoded[name] = tensor
    return encoded, shapes

def delta_decode_tensors(quant_result: dict[str, Tensor], shapes: dict[str, tuple]) -> dict[str, Tensor]:
    """Reverse delta encoding via cumulative sum."""
    decoded: dict[str, Tensor] = {}
    for name, tensor in quant_result.items():
        if name in shapes:
            flat = tensor.flatten().to(torch.int16)
            original = torch.cumsum(flat, dim=0).to(torch.int8)
            decoded[name] = original.reshape(shapes[name])
        else:
            decoded[name] = tensor
    return decoded
...
def main() -> None:
    ...
    # --- Strategy 1: plain (baseline) ---
    quant_buf_plain = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf_plain)
    quant_raw_plain = quant_buf_plain.getvalue()
    quant_blob_plain = zstandard.ZstdCompressor(level=22).compress(quant_raw_plain) if _COMPRESSOR == "zstd" else zlib.compress(quant_raw_plain, 9)
    # --- Strategy 2: delta encoding on .q tensors ---
    delta_result, delta_shapes = delta_encode_tensors(quant_result)
    quant_buf_delta = io.BytesIO()
    torch.save({"w": delta_result, "m": quant_meta, "ds": delta_shapes}, quant_buf_delta)
    quant_raw_delta = quant_buf_delta.getvalue()
    quant_blob_delta = zstandard.ZstdCompressor(level=22).compress(quant_raw_delta) if _COMPRESSOR == "zstd" else zlib.compress(quant_raw_delta, 9)
    # Pick the smaller blob
    if len(quant_blob_delta) < len(quant_blob_plain):
        quant_blob = quant_blob_delta
        if master_process:
            log0(f"Delta encoding saved {len(quant_blob_plain) - len(quant_blob_delta)} bytes "
                 f"(plain={len(quant_blob_plain)}, delta={len(quant_blob_delta)})")
    else:
        quant_blob = quant_blob_plain
        if master_process:
            log0(f"Plain encoding smaller (plain={len(quant_blob_plain)}, delta={len(quant_blob_delta)})")
    if master_process:
    ...
    quant_w = quant_state["w"]
    # If delta-encoded, reverse the encoding
    if "ds" in quant_state:
        quant_w = delta_decode_tensors(quant_w, quant_state["ds"])
        if master_process:
            log0("Decoded delta-encoded quantized weights")
    deq_state = dequantize_mixed_int6(quant_w, quant_state["m"], sd_cpu)
    eval_model = GPT(
