...
def eval_val_sliding_ttt(
    ...
    for ci in range(num_chunks):
        ...
        # --- Compute surprise weights for this chunk's training ---
        chunk_surprise = None
        is_last_chunk = (ci == num_chunks - 1)
        if not is_last_chunk and args.ttt_epochs > 0:
            chunk_start_tok = ci * ttt_chunk
            chunk_end_tok = min((ci + 1) * ttt_chunk, total_tokens)
            chunk_seqs_for_surprise = (chunk_end_tok - chunk_start_tok) // seq_len
            if chunk_seqs_for_surprise > 0:
                with torch.no_grad():
                    surprise_end = chunk_start_tok + chunk_seqs_for_surprise * seq_len + 1
                    if surprise_end <= val_tokens.numel():
                        s_local = val_tokens[chunk_start_tok:surprise_end].to(device=device, dtype=torch.int64)
                        s_x = s_local[:-1].reshape(-1, seq_len)
                        s_y = s_local[1:].reshape(-1, seq_len)
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            s_logits = base_model.forward_logits(s_x)
                        s_nll = F.cross_entropy(
                            s_logits.reshape(-1, s_logits.size(-1)).float(),
                            s_y.reshape(-1), reduction="none"
                        ).reshape(chunk_seqs_for_surprise, seq_len)
                        # Per-sequence mean surprise, normalized to weights
                        seq_surprise = s_nll.mean(dim=1)  # (num_seqs,)
                        # Softmax to get weights (higher surprise = higher weight)
                        chunk_surprise = F.softmax(seq_surprise / 0.5, dim=0) * chunk_seqs_for_surprise  # scale so mean=1
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        # Apply surprise weights if available
                        if chunk_surprise is not None:
                            batch_idx_start = my_seq_s + bs
                            batch_idx_end = my_seq_s + be
                            if batch_idx_end <= chunk_surprise.size(0):
                                weights = chunk_surprise[batch_idx_start:batch_idx_end].to(per_seq_loss.device)
                                loss = (per_seq_loss * weights).mean()
                            else:
