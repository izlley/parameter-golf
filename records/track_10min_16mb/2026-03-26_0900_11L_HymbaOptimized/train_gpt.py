...
class Hyperparameters:
    ...
    hymba_ssm_heads = int(os.environ.get("HYMBA_SSM_HEADS", 2))  # SSM heads (2 of 8 → direct concat)
    ...
    hymba_layers = os.environ.get("HYMBA_LAYERS", "5,6,7,8,9,10")  # which layers use Hymba (empty=all)
...
class HybridAttention(nn.Module):
    """Optimized Hymba-style: split heads into attention + SSM.
    With num_ssm_heads chosen so attn_dim + ssm_dim = dim, enabling direct concat
    (no up-projection needed). E.g., 6 attn heads (384) + 2 SSM heads (128) = 512."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        # Direct concat: attn_dim + ssm_dim should == dim (no up-projection needed)
        assert attn_dim + ssm_dim == dim, (
            f"attn_dim({attn_dim}) + ssm_dim({ssm_dim}) != dim({dim}). "
            f"Adjust num_ssm_heads so heads split evenly."
        )

        # Output projection from concatenated output
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        attn_out = attn_out.reshape(bsz, seqlen, self.attn_dim)  # (bsz, seqlen, attn_dim)

        # === SSM path ===
        ssm_out = self.ssm(self.ssm_proj_in(x))  # (bsz, seqlen, ssm_dim)

        # === Direct concatenation (no up-projection, no LayerNorm averaging) ===
...
class GPT(nn.Module):
    def __init__(
        ...
        # Parse which layers use Hymba (empty string = all layers when enabled)
        if hymba_enabled and hymba_layers.strip():
            self.hymba_layer_set = {int(x.strip()) for x in hymba_layers.split(",") if x.strip()}
        elif hymba_enabled:
            self.hymba_layer_set = set(range(num_layers))
        else:
            self.hymba_layer_set = set()
        if hymba_enabled:
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    hymba=(i in self.hymba_layer_set),
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        hymba_layers=args.hymba_layers,
    ).to(device).bfloat16()
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
    # Keep mamba_ssm Mamba params in float32 for optimizer precision
    for module in base_model.modules():
    ...
    hymba_indices = sorted(base_model.hymba_layer_set) if base_model.hymba_layer_set else []
    log0(f"hymba:enabled={args.hymba_enabled} ssm_heads={args.hymba_ssm_heads} "
         f"d_state={args.hymba_d_state} d_conv={args.hymba_d_conv} "
         f"hymba_layers={hymba_indices} pure_attn_layers={[i for i in range(args.num_layers) if i not in base_model.hymba_layer_set]}")
