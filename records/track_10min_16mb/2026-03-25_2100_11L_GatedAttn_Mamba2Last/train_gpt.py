...
class Hyperparameters:
    ...
    mamba_layers = os.environ.get("MAMBA_LAYERS", "10")
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,ssm_scale,dt_bias,A_log,.D,attn_gate",
    ).split(",")
    if pattern
)
...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.proj is not None:
            h = self.proj(h)
        return h * self.scale.to(dtype=h.dtype)
# ===== Mamba2 / SSD (Pure PyTorch, no einops, no custom CUDA) =====
...
def _ssd_chunked(x: Tensor, A: Tensor, B: Tensor, C: Tensor, chunk_size: int) -> Tensor:
    ...
    x: (B, L, H, P)   — input (head-dim P)
    A: (B, L, H)       — diagonal state matrix (log-space after softplus)
    ...
    decay_chunk = torch.exp(
        _segsum(F.pad(A_cumsum[:, :, :, -1], (1, 0)))           # (B, H, C+1, C+1)
    )
    new_states = torch.einsum("bhzc, bchpn -> bzhpn", decay_chunk, states)
    states = new_states[:, :-1]                                 # (B, C, H, P, N)

    # 4. State-to-output (C terms with decay)
    state_decay = torch.exp(A_cumsum)                           # (B, H, C, L)
...
class Mamba2Layer(nn.Module):
    ...
    Input: (batch, seqlen, dim), Output: (batch, seqlen, dim)."""
    def __init__(self, d_model: int, d_state: int = 64, d_conv: int = 4,
                 expand: int = 2, chunk_size: int = 64):
        super().__init__()
    ...
    def _init_params(self):
        ...
        # A_log init: log of uniform in [1, 16] — standard Mamba2 init
        with torch.no_grad():
    ...
    def forward(self, u: Tensor, v_embed: Tensor | None = None) -> Tensor:
        """u: (B, L, D). v_embed is ignored (compatibility with attention API)."""
        B, L, _ = u.shape
        cs = self.chunk_size
        ...
        Bm = Bm.unsqueeze(2).expand(-1, -1, self.nheads, -1)   # (B, L, H, N)
        Cm = Cm.unsqueeze(2).expand(-1, -1, self.nheads, -1)   # (B, L, H, N)
        # SSD core
...
class Mamba2Block(nn.Module):
    """Block that uses Mamba2 instead of attention, keeping MLP."""
    def __init__(self, dim: int, mlp_mult: int, d_state: int = 64, d_conv: int = 4,
                 expand: int = 2, chunk_size: int = 64, layer_idx: int = 0,
                 ln_scale: bool = False):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(
        ...
        mamba_layer_set = set()
        if mamba_layers:
            mamba_layer_set = {int(x.strip()) for x in mamba_layers.split(",") if x.strip()}
        blocks: list[nn.Module] = []
        for i in range(num_layers):
            if i in mamba_layer_set:
                blocks.append(Mamba2Block(
                    model_dim, mlp_mult, d_state=mamba_d_state, d_conv=mamba_d_conv,
                ...
                blocks.append(Block(
                    ...
                    rope_base, qk_gain_init, layer_idx=i, ln_scale=ln_scale, dtg=dtg,
        ...
        if rope_dims > 0:
            ...
            for i, block in enumerate(self.blocks):
                if i not in mamba_layer_set and hasattr(block, 'attn'):
        ...
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                if i not in mamba_layer_set and hasattr(self.blocks[i], 'attn'):
...
def _classify_param(name: str) -> str:
    ...
    if ".mlp." in name:
        return "mlp"
    if ".ssm." in name:
        return "attn"  # treat SSM projection weights same as attn for quantization
...
def main() -> None:
    ...
    mamba_indices = sorted(base_model.mamba_layer_set) if base_model.mamba_layer_set else []
    log0(f"hybrid_arch:mamba_layers={mamba_indices} attn_layers={[i for i in range(args.num_layers) if i not in base_model.mamba_layer_set]} gated_attention:{args.gated_attention}")
