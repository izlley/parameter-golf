...
def eval_val_sliding_ttt(
    ...
    # --- Supermask setup: channel-level masks on MLP hidden + Attn output dims ---
    # Freeze ALL model weights
    for p in base_model.parameters():
        p.requires_grad_(False)

    # Create mask scores for each block: MLP hidden + Attn output
    ...
    for bi, block in enumerate(base_model.blocks):
        ...
        block.mlp.forward = _make_masked_mlp_fwd(fc_layer, proj_layer, mlp_ms)
        # Monkey-patch Attn forward: apply channel mask to output
        orig_attn_fwd = block.attn.forward.__func__ if hasattr(block.attn.forward, '__func__') else None
        def _make_masked_attn_fwd(attn_module, ms, orig_fwd):
        ...
        if orig_attn_fwd is not None:
            block.attn.forward = _make_masked_attn_fwd(block.attn, attn_ms, orig_attn_fwd)

    total_mask_params = sum(ms.numel() for ms in mask_scores)
    total_frozen = sum(p.numel() for p in base_model.parameters())
    num_mlp_masks = sum(1 for i in range(0, len(mask_scores), 2))
    num_attn_masks = sum(1 for i in range(1, len(mask_scores), 2))
    ...
    log0(f"supermask_ttt:params mask_params={total_mask_params} "
         f"(mlp_masks={num_mlp_masks} attn_masks={num_attn_masks}) model_frozen={total_frozen}")
