...
def eval_val_sliding_ttt(
    ...
    """LoRA TTT: freeze all model weights, learn rank-1 LoRA adapters on MLP proj + Attn proj.
    Score-first (legal): score each chunk before any LoRA update."""
    ...
    # --- LoRA TTT setup: rank-1 LoRA adapters on MLP proj + Attn proj ---
    # Freeze ALL model weights
    for p in base_model.parameters():
        p.requires_grad_(False)

    # Create rank-1 LoRA adapters for each block's MLP.proj and Attn.proj
    # LoRA: output += (x @ A) @ B where A: [in, r], B: [r, out], r=1
    lora_params: list[Tensor] = []
    original_mlp_forwards: list = []
    original_attn_forwards: list = []
    lora_rank = 1
    for block in base_model.blocks:
        mlp_in = block.mlp.proj.weight.shape[1]   # hidden dim (e.g., 1536)
        mlp_out = block.mlp.proj.weight.shape[0]   # model dim (e.g., 512)
        attn_in = block.attn.proj.weight.shape[1]   # model dim
        attn_out = block.attn.proj.weight.shape[0]  # model dim
        # MLP LoRA: A=[hidden, 1], B=[1, dim]
        mlp_A = torch.zeros(mlp_in, lora_rank, device=device, requires_grad=True)
        mlp_B = torch.zeros(lora_rank, mlp_out, device=device, requires_grad=True)
        # Attn LoRA: A=[dim, 1], B=[1, dim]
        attn_A = torch.zeros(attn_in, lora_rank, device=device, requires_grad=True)
        attn_B = torch.zeros(lora_rank, attn_out, device=device, requires_grad=True)
        lora_params.extend([mlp_A, mlp_B, attn_A, attn_B])
        original_mlp_forwards.append(block.mlp.forward)
        original_attn_forwards.append(block.attn.forward)

    # Monkey-patch forwards with LoRA versions
    for bi, block in enumerate(base_model.blocks):
        mlp_A = lora_params[bi * 4]
        mlp_B = lora_params[bi * 4 + 1]
        attn_A = lora_params[bi * 4 + 2]
        attn_B = lora_params[bi * 4 + 3]
        fc_layer = block.mlp.fc
        proj_layer = block.mlp.proj
        def _make_lora_mlp_fwd(fc, proj, A, B):
            def lora_forward(x):
                h = F.leaky_relu(fc(x), negative_slope=0.5)
                h = h.square()
                out = proj(h)
                # LoRA additive: out += h @ A @ B
                lora_out = (h.to(A.dtype) @ A) @ B
                return out + lora_out.to(out.dtype)
            return lora_forward
        block.mlp.forward = _make_lora_mlp_fwd(fc_layer, proj_layer, mlp_A, mlp_B)
        orig_attn_fwd = block.attn.forward.__func__ if hasattr(block.attn.forward, '__func__') else None
        def _make_lora_attn_fwd(attn_module, A, B, orig_fwd):
            def lora_attn_forward(x, v_embed=None):
                out = orig_fwd(attn_module, x, v_embed=v_embed)
                # LoRA additive on proj output
                lora_out = (out.to(A.dtype) @ A) @ B
                return out + lora_out.to(out.dtype)
            return lora_attn_forward
        if orig_attn_fwd is not None:
            block.attn.forward = _make_lora_attn_fwd(block.attn, attn_A, attn_B, orig_attn_fwd)

    total_mask_params = sum(ms.numel() for ms in lora_params)
    total_frozen = sum(p.numel() for p in base_model.parameters())
    num_blocks = len(base_model.blocks)
    log0(f"lora_ttt:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         f"total_windows={len(window_starts)} stride={stride} "
         f"ttt_lr={args.ttt_lr} ttt_epochs={args.ttt_epochs}")
    log0(f"lora_ttt:params lora_params={total_mask_params} "
         f"(blocks={num_blocks} per_block=mlp_A+mlp_B+attn_A+attn_B rank={lora_rank}) model_frozen={total_frozen}")

    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)

    optimizer = torch.optim.SGD(lora_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        loss.backward()
                        if dist.is_available() and dist.is_initialized():
                            for ms in lora_params:
                                if ms.grad is not None:
                                    dist.all_reduce(ms.grad, op=dist.ReduceOp.AVG)
                        torch.nn.utils.clip_grad_norm_(lora_params, args.ttt_grad_clip)
                        optimizer.step()
    ...
    log0(f"lora_ttt:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
