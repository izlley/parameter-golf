...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Differential Attention: learnable lambda per head (init small for identity-like start)
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        # Differential Attention: double the heads, halve the dim for Q/K/V
        half_d = self.head_dim // 2
        q_double = q.reshape(bsz, seqlen, self.num_heads * 2, half_d)
        k_double = k.reshape(bsz, seqlen, self.num_kv_heads * 2, half_d)
        v_double = v.reshape(bsz, seqlen, self.num_kv_heads * 2, half_d)
        # Single flash attention call with 2x heads (same FLOPS: 2*H heads * D/2 dim)
        y_double = flash_attn_3_func(q_double, k_double, v_double, causal=True)
        ...
        y = torch.cat([y_diff, y_sum], dim=-1)  # [B, T, H, D]
        if self.use_xsa:
