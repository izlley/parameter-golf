...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,iter_scales,depth_router",
    ).split(",")
    if pattern
)
...
class DepthRouter(nn.Module):
    """Lightweight router: decides if a token needs additional recursion."""
    def __init__(self, dim: int):
        super().__init__()
        self.proj = nn.Linear(dim, 1, bias=True)
        nn.init.zeros_(self.proj.weight)
        nn.init.constant_(self.proj.bias, 0.0)  # Start at 50% routing
    def forward(self, x: Tensor, temperature: float = 1.0) -> Tensor:
        """Returns routing probability (B, T, 1). Higher = more recursion."""
        logits = self.proj(x.detach())  # Detach to avoid routing affecting representation gradients
        if self.training:
            # Gumbel-sigmoid for differentiable discrete routing
...
class GPT(nn.Module):
    def __init__(
        ...
        # MoR: Layer 8 and 9 share the same block (recursive with skip integration)
        self.mor_layer_idx = 8  # Layer 8 is the recursive block
        self.mor_max_repeat = 2
        self.blocks[self.mor_layer_idx + 1] = self.blocks[self.mor_layer_idx]  # Share block 9 = block 8
        self.depth_router = DepthRouter(model_dim)
        self.iter_scales = nn.Parameter(torch.ones(self.mor_max_repeat, model_dim, dtype=torch.float32))
        if rope_dims > 0:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if bi == self.mor_layer_idx:
                # MoR iter1: apply skip from encoder 1, run shared block
                if skips:
                    x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
                ve = self._get_ve(bi, input_ids, ve_cache)
                x_iter1 = self.blocks[bi](x, x0, v_embed=ve)
                x_iter1 = x_iter1 * self.iter_scales[0].to(dtype=x.dtype)[None, None, :]
                # Router decides per-token recursion depth
                route_prob = self.depth_router(x_iter1)
                # MoR iter2: apply skip from encoder 0, run same block on iter1 output
                if skips:
                    x_iter1_skip = x_iter1 + self.skip_weights[i + 1].to(dtype=x.dtype)[None, None, :] * skips.pop()
                else:
                    x_iter1_skip = x_iter1
                ve2 = self._get_ve(bi + 1, input_ids, ve_cache)
                x_iter2 = self.blocks[bi](x_iter1_skip, x0, v_embed=ve2)
                x_iter2 = x_iter2 * self.iter_scales[1].to(dtype=x.dtype)[None, None, :]
                # Blend: route_prob=1 means use iter2, route_prob=0 means use iter1
                x = route_prob * x_iter2 + (1 - route_prob) * x_iter1
            elif bi == self.mor_layer_idx + 1:
                # Skip: already handled by MoR above (layer 9 is same block as layer 8)
                pass
            else:
                if skips:
                    x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
...
def main() -> None:
    ...
    # MoR router and iteration scale params
    for p in base_model.depth_router.parameters():
