...
def main() -> None:
    ...
    # Also try zstd and zlib for comparison logging
    zstd_blob = zstandard.ZstdCompressor(level=22).compress(quant_raw) if zstandard else None
    zlib_blob = zlib.compress(quant_raw, 9)
    if master_process:
        ...
        if zstd_blob is not None:
            log0(f"Comparison zstd-22: {len(zstd_blob)} bytes (model only)")
        log0(f"Comparison zlib-9: {len(zlib_blob)} bytes (model only)")
    ...
    quant_state = torch.load(
        io.BytesIO(brotli.decompress(quant_blob_disk) if _COMPRESSOR == "brotli" else (zstandard.ZstdDecompressor().decompress(quant_blob_disk) if _COMPRESSOR == "zstd" else zlib.decompress(quant_blob_disk))),
        map_location="cpu",
    )
