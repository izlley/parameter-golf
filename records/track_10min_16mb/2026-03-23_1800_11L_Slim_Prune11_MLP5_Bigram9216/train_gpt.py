...
class Hyperparameters:
    ...
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 9216))
    ...
    encoder_kv_heads = int(os.environ.get("ENCODER_KV_HEADS", 2))
    encoder_mlp_mult = float(os.environ.get("ENCODER_MLP_MULT", 2.75))
    encoder_mlp_layers = int(os.environ.get("ENCODER_MLP_LAYERS", 5))
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int = 128,
        encoder_kv_heads: int = 0,
        encoder_mlp_mult: float = 0.0,
        encoder_mlp_layers: int = 0,
    ):
        super().__init__()
        ...
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, encoder_kv_heads if (encoder_kv_heads > 0 and i < self.num_encoder_layers) else num_kv_heads, encoder_mlp_mult if (encoder_mlp_mult > 0 and i < encoder_mlp_layers) else mlp_mult, rope_base, qk_gain_init)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        encoder_kv_heads=args.encoder_kv_heads,
        encoder_mlp_mult=args.encoder_mlp_mult,
    ...
    with torch.no_grad():
        for name, param in base_model.named_parameters():
            if param.ndim == 2 and param.numel() > 65536:
                threshold = torch.quantile(param.abs().float().flatten(), 0.11)
