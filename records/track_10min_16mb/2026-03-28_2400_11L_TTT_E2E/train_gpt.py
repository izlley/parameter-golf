...
class Hyperparameters:
    ...
    e2e_ttt_interval = int(os.environ.get("E2E_TTT_INTERVAL", 500))
    e2e_ttt_inner_steps = int(os.environ.get("E2E_TTT_INNER_STEPS", 2))
    e2e_ttt_inner_lr = float(os.environ.get("E2E_TTT_INNER_LR", 0.01))
    e2e_ttt_weight = float(os.environ.get("E2E_TTT_WEIGHT", 0.1))
...
def eval_val_sliding_ttt(
    ...
    optimizer = torch.optim.SGD(mask_scores, lr=args.ttt_lr, momentum=args.ttt_momentum)
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
@torch.no_grad()
def _e2e_collect_mlp_info(raw_model: nn.Module):
    """Collect MLP fc/proj layers and hidden dims for E2E TTT rehearsal."""
    info = []
    for block in raw_model.blocks:
        fc = block.mlp.fc
        proj = block.mlp.proj
        hidden_dim = fc.weight.shape[0]
        info.append((block, fc, proj, hidden_dim))
    return info


def e2e_ttt_rehearsal(
    raw_model: nn.Module, x: Tensor, y: Tensor,
    inner_steps: int = 2, inner_lr: float = 0.01,
) -> Tensor:
    """Simulate TTT during training: learn temp masks, return adapted loss.

    Gradients flow through the inner loop to update model weights.
    Uses raw_model (not compiled/DDP) to avoid torch.compile issues with monkey-patching.
    """
    mlp_info = _e2e_collect_mlp_info(raw_model)

    # Create temporary masks for each block
    temp_masks: list[Tensor] = []
    original_forwards: list = []
    for block, fc, proj, hidden_dim in mlp_info:
        ms = torch.full((hidden_dim,), 3.0, device=x.device, dtype=torch.float32, requires_grad=True)
        temp_masks.append(ms)
        original_forwards.append(block.mlp.forward)

        def _make_fwd(_fc, _proj, _ms):
            def fwd(inp):
                h = F.leaky_relu(_fc(inp), negative_slope=0.5)
                h = h.square()
                mask = torch.sigmoid(_ms).to(h.dtype)
                h = h * mask
                return _proj(h)
            return fwd
    ...
    for _inner in range(inner_steps):
        ...
        for bi, (block, fc, proj, _) in enumerate(mlp_info):
            def _make_fwd2(_fc, _proj, _ms):
                def fwd(inp):
                    h = F.leaky_relu(_fc(inp), negative_slope=0.5)
                    h = h.square()
                    mask = torch.sigmoid(_ms).to(h.dtype)
                    ...
                    return _proj(h)
                return fwd
            block.mlp.forward = _make_fwd2(fc, proj, temp_masks[bi])

    # Final forward with adapted masks — this loss trains the model weights
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        final_logits = raw_model.forward_logits(x)
    ...
    for (block, _, _, _), orig_fwd in zip(mlp_info, original_forwards):
        block.mlp.forward = orig_fwd
...
def main() -> None:
    ...
    log0(
        f"e2e_ttt:interval={args.e2e_ttt_interval} inner_steps={args.e2e_ttt_inner_steps} "
        f"inner_lr={args.e2e_ttt_inner_lr} weight={args.e2e_ttt_weight}"
    )
    ...
    while True:
        ...
        for opt in optimizers:
            opt.step()
        # E2E TTT rehearsal: simulate TTT during training to optimize model for mask adaptation
        if args.e2e_ttt_interval > 0 and step > 0 and step % args.e2e_ttt_interval == 0:
            zero_grad_all()
            # Use raw (uncompiled) model to avoid torch.compile issues with monkey-patching
            e2e_loss = e2e_ttt_rehearsal(
                base_model, x, y,
                inner_steps=args.e2e_ttt_inner_steps,
                inner_lr=args.e2e_ttt_inner_lr,
            )
            (e2e_loss * grad_scale * args.e2e_ttt_weight).backward()
            if args.grad_clip_norm > 0:
                torch.nn.utils.clip_grad_norm_(base_model.parameters(), args.grad_clip_norm)
            for opt in optimizers:
                opt.step()
            log0(f"e2e_ttt:step={step} meta_loss={e2e_loss.item():.4f}")
        zero_grad_all()
        # EMA update
        with torch.no_grad():
