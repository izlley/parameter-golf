...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
                         fisher_grad_sq: dict[str, Tensor] | None = None):
    # Compute Fisher-weighted sensitivity per tensor to decide int5 vs int6
    fisher_sensitivity: dict[str, float] = {}
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            continue
        if cat not in int6_cats:
            continue
        # Compute int5 quantization error
        q5, s5 = quantize_int5_per_row(t)
        recon5 = q5.float() * s5.float().view(q5.shape[0], *([1] * (q5.ndim - 1)))
        delta_sq = (t.float() - recon5).pow(2)
        if fisher_grad_sq is not None and name in fisher_grad_sq:
            # Fisher-weighted: grad^2 * delta^2
            grad_sq = fisher_grad_sq[name]
            impact = (grad_sq * delta_sq).sum().item()
        else:
            # Fallback to MSE-weighted
            impact = delta_sq.mean().item() * tensor.numel()
        fisher_sensitivity[name] = impact
    # Sort by Fisher impact (ascending = least impactful first)
    sorted_tensors = sorted(fisher_sensitivity.items(), key=lambda x: x[1])
    # Select ~55% of params for int5
    total_params = sum(state_dict[n].numel() for n, _ in sorted_tensors)
    target_int5 = int(total_params * 0.55)
    cumulative = 0
    int5_tensors: set[str] = set()
    for name, impact in sorted_tensors:
        if cumulative >= target_int5:
            break
        int5_tensors.add(name)
        cumulative += state_dict[name].numel()
    print(f"Fisher-EWQ: {len(int5_tensors)}/{len(fisher_sensitivity)} tensors -> int5, "
          f"covering {cumulative}/{total_params} params ({100*cumulative/max(total_params,1):.1f}%)")
    # Quantize
...
def main() -> None:
    ...
    # EWQ: mark early layer CastedLinear modules for int5 QAT
    ...
    fisher_grad_sq: dict[str, Tensor] = {}
    ...
    while True:
        ...
        for opt in optimizers:
            opt.step()
        # Save Fisher information (last gradient squared) before zeroing
        # Only save near end of training to minimize overhead
        if stop_after_step is not None or step + 1 >= args.iterations:
            fisher_grad_sq = {}
            for name, param in base_model.named_parameters():
                if param.grad is not None and param.ndim >= 2 and param.numel() > 65536:
                    if not any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
                        fisher_grad_sq[name] = param.grad.float().pow(2).cpu()
        zero_grad_all()
        # EMA update
        with torch.no_grad():
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, fisher_grad_sq if fisher_grad_sq else None)
