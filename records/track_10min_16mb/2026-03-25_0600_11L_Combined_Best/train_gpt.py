...
def main() -> None:
    ...
    # Collect non-Muon params for manual all-reduce (Muon handles its own gradient sync)
    _muon_param_set = set(id(p) for p in matrix_params)
    replicated_params = [p for p in base_model.parameters() if id(p) not in _muon_param_set]
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            # Manual gradient sync for non-Muon params
            if distributed:
    ...
    while True:
        ...
        # Manual gradient sync for non-Muon params (Muon has its own all-reduce)
        if distributed:
            for p in replicated_params:
                if p.grad is not None:
                    dist.all_reduce(p.grad, op=dist.ReduceOp.AVG)
