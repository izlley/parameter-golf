...
def pack_int6(q_int8: Tensor) -> bytes:
    """Pack int8 tensor with values in [-31, 31] into 6-bit packed bytes.
    Offset to unsigned [0, 62], then pack 4 values into 3 bytes."""
    flat = q_int8.flatten().numpy().astype(np.int8)
    unsigned = (flat.astype(np.int16) + 31).astype(np.uint8)  # [0, 62]
    n = len(unsigned)
    pad = (4 - n % 4) % 4
    if pad:
        unsigned = np.concatenate([unsigned, np.zeros(pad, dtype=np.uint8)])
    # Pack 4 x 6-bit values into 3 bytes: [a,b,c,d] -> 3 bytes
    a, b, c, d = unsigned[0::4], unsigned[1::4], unsigned[2::4], unsigned[3::4]
    b0 = (a << 2) | (b >> 4)             # top 6 of a + top 2 of b
    b1 = ((b & 0x0F) << 4) | (c >> 2)    # bottom 4 of b + top 4 of c
    b2 = ((c & 0x03) << 6) | d           # bottom 2 of c + 6 of d
    packed = np.empty(len(a) * 3, dtype=np.uint8)
    packed[0::3] = b0
    packed[1::3] = b1
    packed[2::3] = b2
    return n, packed.tobytes()

def unpack_int6(n: int, data: bytes) -> Tensor:
    """Unpack 6-bit packed bytes back to int8 tensor with values in [-31, 31]."""
    ...
    c = ((b1 & 0x0F) << 2) | (b2 >> 6)
    d = b2 & 0x3F
    unsigned = np.empty(len(a) * 4, dtype=np.uint8)
    unsigned[0::4] = a
    unsigned[1::4] = b
    unsigned[2::4] = c
    unsigned[3::4] = d
    signed = unsigned[:n].astype(np.int16) - 31
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            n, packed = pack_int6(q)
            result[name + ".packed"] = torch.from_numpy(np.frombuffer(packed, dtype=np.uint8).copy())
            result[name + ".scale"] = s
            meta[name] = {"type": "int6_packed", "n": n, "shape": list(q.shape)}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "int6_packed":
            packed_np = result[name + ".packed"]
            packed_bytes = packed_np.numpy().tobytes() if isinstance(packed_np, Tensor) else bytes(packed_np)
            s = result[name + ".scale"]
            q = unpack_int6(info["n"], packed_bytes).reshape(info["shape"])
            if s.ndim > 0:
