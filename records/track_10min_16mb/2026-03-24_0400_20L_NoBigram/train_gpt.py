...
class Hyperparameters:
    ...
    prune_quantile = float(os.environ.get("PRUNE_QUANTILE", 0.05))
...
class GPT(nn.Module):
    def __init__(
        ...
        if layer_configs is None:
            layer_configs = [(mlp_mult, num_kv_heads)] * num_layers
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, kv_h, mlp_m, rope_base, qk_gain_init)
                for mlp_m, kv_h in layer_configs
            ]
        )
...
def main() -> None:
    ...
    # 20L NoBigram: 12 AttnOnly (MLP 0, KV 2) + 7 Full (MLP 3.0x, KV 4) + 1 Mini (MLP 1.5x, KV 1)
    layer_configs = [(0, 2)] * 12 + [(1.5, 1)] * 1 + [(3.0, 4)] * 7
    log0(f"layer_configs: {layer_configs}")
    base_model = GPT(
    ...
    if args.prune_quantile > 0:
        with torch.no_grad():
            for name, param in base_model.named_parameters():
                if param.ndim == 2 and param.numel() > 65536:
                    threshold = torch.quantile(param.abs().float().flatten(), args.prune_quantile)
