...
class Hyperparameters:
    ...
    ttt_warmup_frac = float(os.environ.get("TTT_WARMUP_FRAC", 0.02))
    ttt_constant_frac = float(os.environ.get("TTT_CONSTANT_FRAC", 0.20))
    ttt_lr_min_ratio = float(os.environ.get("TTT_LR_MIN_RATIO", 0.05))
...
def eval_val_sliding_ttt(
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                # Warmup + short constant + long cosine decay (aggressive early decay)
                frac = ci / max(num_chunks - 1, 1)
                warmup_end = args.ttt_warmup_frac
                constant_end = warmup_end + args.ttt_constant_frac
                lr_min = args.ttt_lr * args.ttt_lr_min_ratio
                if frac < warmup_end:
                    cos_lr = args.ttt_lr * (frac / warmup_end)  # linear warmup
                elif frac < constant_end:
                    cos_lr = args.ttt_lr  # constant
                else:
                    decay_frac = (frac - constant_end) / (1.0 - constant_end)
                    cos_lr = lr_min + (args.ttt_lr - lr_min) * 0.5 * (1.0 + math.cos(math.pi * decay_frac))
