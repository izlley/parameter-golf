...
class Hyperparameters:
    ...
    num_blocks = int(os.environ.get("NUM_BLOCKS", 10))  # unique stored blocks
    # Effective layer execution pattern: block indices, can repeat for weight sharing
    # Default: [0,1,2,3,4,5,3,4,5,6,7,8,9] = 13 effective layers from 10 blocks
    _default_layer_pattern = "0,1,2,3,4,5,3,4,5,6,7,8,9"
    layer_pattern = list(map(int, os.environ.get("LAYER_PATTERN", _default_layer_pattern).split(",")))
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int = 128,
        layer_pattern: list[int] | None = None,
    ):
        super().__init__()
        ...
        # Weight sharing: layer_pattern defines execution order (block indices, can repeat)
        self.layer_pattern = layer_pattern if layer_pattern is not None else list(range(num_blocks))
        effective_layers = len(self.layer_pattern)
    ...
    def _init_weights(self) -> None:
        ...
        # Scale output projections by effective depth (not stored blocks)
        effective_layers = len(self.layer_pattern)
...
def main() -> None:
    ...
    log0(f"weight_sharing: {args.num_blocks} unique blocks, {len(args.layer_pattern)} effective layers")
    log0(f"layer_pattern: {args.layer_pattern}")
