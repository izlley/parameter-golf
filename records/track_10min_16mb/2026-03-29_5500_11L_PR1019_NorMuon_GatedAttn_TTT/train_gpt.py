...
class Hyperparameters:
    ...
    gated_attention = bool(int(os.environ.get("GATED_ATTENTION", "1")))  # enabled (our finding)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                # NorMuon: neuron-wise L2 normalization after Newton-Schulz
                if update.ndim == 3:
                    update = F.normalize(update, dim=-1)  # normalize along cols for batched 3D
                elif update.ndim == 2:
                    update = F.normalize(update, dim=1)   # normalize along cols for 2D
...
# --- MuonLiteTTT optimizer for test-time training ---

class MuonLiteTTT(torch.optim.Optimizer):
    """Lightweight SGD+momentum optimizer for TTT supermask parameters."""
    def __init__(self, params, lr: float = 0.002, momentum: float = 0.9, grad_clip: float = 1.0):
        super().__init__(params, dict(lr=lr, momentum=momentum, grad_clip=grad_clip))
...
def _ttt_lr_schedule(chunk_idx: int, total_chunks: int, base_lr: float) -> float:
    """LR schedule across chunks: warmup 5% -> constant 65% -> cosine 30%."""
    if total_chunks <= 1:
        return base_lr
    frac = chunk_idx / total_chunks
    if frac < 0.05:
        return base_lr * (frac / 0.05)
    elif frac < 0.70:
        return base_lr
    else:
        cos_frac = (frac - 0.70) / 0.30
        return base_lr * 0.5 * (1.0 + math.cos(math.pi * cos_frac))
...
def eval_val_sliding_ttt(
    ...
    """Sliding window evaluation with SupermaskTTT adapted for Parameter Banking.

    For each chunk of validation data, learns lightweight supermask parameters
    (per-block MLP and attention masks + biases) that modulate outputs.
    """
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    num_layers = base_model.num_layers
    model_dim = args.model_dim
    hidden_dim = int(args.mlp_mult * model_dim)

    # Chunk validation tokens
    ...
    my_chunk_start = (num_full_chunks * rank) // world_size
    my_chunk_end = (num_full_chunks * (rank + 1)) // world_size
    ...
    for chunk_idx in range(my_chunk_start, my_chunk_end):
        ...
        for bi in range(num_layers):
            ms_mlp = torch.full((hidden_dim,), 3.0, device=device, dtype=torch.float32, requires_grad=True)
            bias_mlp = torch.zeros(hidden_dim, device=device, dtype=torch.float32, requires_grad=True)
            ms_attn = torch.full((model_dim,), 3.0, device=device, dtype=torch.float32, requires_grad=True)
            bias_attn = torch.zeros(model_dim, device=device, dtype=torch.float32, requires_grad=True)
            mlp_masks.append(ms_mlp)
            mlp_biases.append(bias_mlp)
            attn_masks.append(ms_attn)
            attn_biases.append(bias_attn)
            ttt_params.extend([ms_mlp, bias_mlp, ms_attn, bias_attn])
        ...
        # Monkey-patch the model's forward methods
        original_mlp_fwds = []
        original_attn_fwds = []
        ...
        for bi in range(num_layers):
            ...
            # Save originals
            original_mlp_fwds.append(block.mlp.forward)
            original_attn_fwds.append(block.attn.forward)

            # Patch MLP
            ms_m = mlp_masks[bi]
            bias_m = mlp_biases[bi]
            def _make_masked_mlp_fwd(original_mlp, ms, bias):
                def masked_forward(x, up_w, down_w):
            ...
            block.mlp.forward = _make_masked_mlp_fwd(block.mlp, ms_m, bias_m)

            # Patch Attn
            ms_a = attn_masks[bi]
            bias_a = attn_biases[bi]
            orig_attn_fwd = original_attn_fwds[bi]
            def _make_masked_attn_fwd(attn_module, ms, bias, orig_fwd):
                def masked_attn_forward(x, q_w, k_w, v_w, out_w, v_embed=None, v0=None):
                    out, raw_v = orig_fwd(x, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
                    mask = torch.sigmoid(ms).to(out.dtype)
                    out = out * mask + bias.to(out.dtype)
                    return out, raw_v
                return masked_attn_forward
            block.attn.forward = _make_masked_attn_fwd(block.attn, ms_a, bias_a, orig_attn_fwd)

        # TTT training loop on this chunk
        chunk_len = chunk_end - chunk_start
        num_seqs = max(1, chunk_len // seq_len)
        usable = num_seqs * seq_len
        chunk_data = chunk_tokens[:usable + 1].to(device=device, dtype=torch.int64)

        lr_now = _ttt_lr_schedule(chunk_idx - my_chunk_start, my_chunk_end - my_chunk_start, args.ttt_lr)
        for g in ttt_opt.param_groups:
            g["lr"] = lr_now

        for epoch in range(args.ttt_epochs):
            for batch_start in range(0, num_seqs, args.ttt_batch_seqs):
                batch_end = min(batch_start + args.ttt_batch_seqs, num_seqs)
                bsz = batch_end - batch_start
                start_tok = batch_start * seq_len
                end_tok = batch_end * seq_len + 1
                local = chunk_data[start_tok:end_tok]
        ...
        # Evaluate this chunk with sliding window using the adapted model
        window_starts = [ws for ws in range(0, chunk_len, stride)
                         if min(ws + seq_len, chunk_len) - ws >= 1]

        with torch.inference_mode():
            for bi_start in range(0, len(window_starts), batch_seqs):
                batch_ws = window_starts[bi_start:bi_start + batch_seqs]
                bsz = len(batch_ws)
                x_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
                y_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
                wlens = []
        ...
        for bi in range(num_layers):
            base_model.blocks[bi].mlp.forward = original_mlp_fwds[bi]
            base_model.blocks[bi].attn.forward = original_attn_fwds[bi]

        # Free TTT params
        del ttt_params, mlp_masks, mlp_biases, attn_masks, attn_biases, ttt_opt
...
def main() -> None:
    ...
    if args.ttt_enabled:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_ttt_sliding_window val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
            f"stride:{args.eval_stride} ttt_lr:{args.ttt_lr} ttt_epochs:{args.ttt_epochs} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms"
        )
        log0(f"final_int6_ttt_sliding_window_exact val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f}")
    if distributed:
