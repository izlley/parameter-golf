...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def gptq_quantize_int6(W: Tensor, H_diag: Tensor, clip_range: int = 31, group_size: int = 8) -> tuple[Tensor, Tensor]:
    """Column-wise GPTQ quantization with diagonal Hessian error compensation."""
    W = W.float().clone()
    n_rows, n_cols = W.shape
    row_max = W.abs().amax(dim=1)
    scale = (row_max / clip_range).clamp_min(1e-8).to(torch.float16)
    Q = torch.zeros_like(W, dtype=torch.int8)
    for col_start in range(0, n_cols, group_size):
        col_end = min(col_start + group_size, n_cols)
        for j in range(col_start, col_end):
            w_col = W[:, j]
            q_col = torch.clamp(torch.round(w_col / scale.float()), -clip_range, clip_range)
            Q[:, j] = q_col.to(torch.int8)
            err = w_col - q_col * scale.float()
            if j + 1 < col_end:
                h_jj = H_diag[j].clamp_min(1e-8)
                remaining_h = H_diag[j+1:col_end]
                correction = err.unsqueeze(1) * (remaining_h / h_jj).unsqueeze(0)
                W[:, j+1:col_end] += correction
    return Q, scale
def collect_gptq_hessian(model: nn.Module, val_tokens: Tensor, device: torch.device,
                         n_batches: int = 8, seq_len: int = 2048) -> dict[str, Tensor]:
    """Run a few val batches and collect X^T X diagonal per Linear layer."""
    hessian_dict: dict[str, Tensor] = {}
    hooks = []
    n_samples_dict: dict[str, int] = {}
    def make_hook(layer_name: str):
        def hook_fn(module, input, output):
            x = input[0].detach().float()  # (B, T, D) or (B*T, D)
            if x.ndim == 3:
                x = x.reshape(-1, x.size(-1))  # flatten to (N, D)
            xtx_diag = (x * x).sum(dim=0)  # diagonal of X^T X
            if layer_name not in hessian_dict:
                hessian_dict[layer_name] = xtx_diag
                n_samples_dict[layer_name] = x.size(0)
            else:
                hessian_dict[layer_name] += xtx_diag
                n_samples_dict[layer_name] += x.size(0)
        return hook_fn
    # Register hooks on all CastedLinear (nn.Linear) layers
    for name, module in model.named_modules():
        if isinstance(module, (CastedLinear, nn.Linear)) and hasattr(module, 'weight') and module.weight.ndim == 2:
            h = module.register_forward_hook(make_hook(name + ".weight"))
            hooks.append(h)
    # Run forward passes on val data
    model.eval()
    total_tokens = val_tokens.numel() - 1
    with torch.no_grad():
        for i in range(n_batches):
            start = i * seq_len
            if start + seq_len + 1 > total_tokens:
                break
    ...
    for h in hooks:
        h.remove()
    # Average the hessian diagonals
    for layer_name in hessian_dict:
        n = n_samples_dict.get(layer_name, 1)
        if n > 0:
            hessian_dict[layer_name] = hessian_dict[layer_name] / n
    model.train()
    return hessian_dict
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessian_dict: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            # Use GPTQ if we have a matching hessian diagonal for this param
            if hessian_dict is not None and name in hessian_dict and t.ndim == 2:
                h_diag = hessian_dict[name].cpu()
...
def main() -> None:
    ...
    # Collect GPTQ Hessian diagonals from validation data
    log0("gptq:collecting Hessian diagonals from val data")
    torch.cuda.synchronize()
    t_gptq = time.perf_counter()
    gptq_hessian = collect_gptq_hessian(base_model, val_tokens, device, n_batches=8, seq_len=2048)
    torch.cuda.synchronize()
    log0(f"gptq:collected {len(gptq_hessian)} layer hessians in {1000.0 * (time.perf_counter() - t_gptq):.0f}ms")
    sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, hessian_dict=gptq_hessian)
