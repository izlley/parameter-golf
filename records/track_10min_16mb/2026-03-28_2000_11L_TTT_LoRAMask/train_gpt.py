...
def eval_val_sliding_ttt(
    ...
    # Monkey-patch forwards with masked+biased+LoRA versions
    for bi, block in enumerate(base_model.blocks):
        mlp_ms = mask_scores[bi * 8]        # MLP mask
        mlp_bias = mask_scores[bi * 8 + 1]  # MLP bias
        attn_ms = mask_scores[bi * 8 + 2]   # Attn mask
        attn_bias = mask_scores[bi * 8 + 3] # Attn bias
        mlp_lora_A = mask_scores[bi * 8 + 4]
        mlp_lora_B = mask_scores[bi * 8 + 5]
        attn_lora_A = mask_scores[bi * 8 + 6]
        attn_lora_B = mask_scores[bi * 8 + 7]
        fc_layer = block.mlp.fc
        proj_layer = block.mlp.proj
        def _make_masked_mlp_fwd(fc, proj, ms, bias, lora_A, lora_B):
            def masked_forward(x):
                ...
                out = proj(h)
                # LoRA correction: h (batch, seq, hidden) @ A (hidden, 1) @ B (1, dim) -> (batch, seq, dim)
                lora_out = (h @ lora_A.to(h.dtype)) @ lora_B.to(h.dtype)
                return out + lora_out
            return masked_forward
        block.mlp.forward = _make_masked_mlp_fwd(fc_layer, proj_layer, mlp_ms, mlp_bias, mlp_lora_A, mlp_lora_B)
        # Monkey-patch Attn forward: apply channel mask + bias + LoRA to output
        orig_attn_fwd = block.attn.forward.__func__ if hasattr(block.attn.forward, '__func__') else None
        def _make_masked_attn_fwd(attn_module, ms, bias, lora_A, lora_B, orig_fwd):
            def masked_attn_forward(x, v_embed=None):
                ...
                # LoRA correction
                lora_out = (out @ lora_A.to(out.dtype)) @ lora_B.to(out.dtype)
                return out + lora_out
            return masked_attn_forward
        if orig_attn_fwd is not None:
            block.attn.forward = _make_masked_attn_fwd(block.attn, attn_ms, attn_bias, attn_lora_A, attn_lora_B, orig_attn_fwd)
    ...
    log0(f"supermask_ttt:params mask_params={total_mask_params} "
         f"(blocks={num_blocks} per_block=mask+bias+lora x2) model_frozen={total_frozen}")
