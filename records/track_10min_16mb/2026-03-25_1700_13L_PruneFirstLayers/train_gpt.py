...
class Hyperparameters:
    ...
    prune_at_frac = float(os.environ.get("PRUNE_AT_FRAC", 0.50))  # prune at 50% of wallclock
    prune_first_layers = bool(int(os.environ.get("PRUNE_FIRST_LAYERS", "1")))  # force remove layers [0,1]
...
def measure_block_influence(
    ...
    BI = 1 - cos_sim(input_to_layer, output_of_layer), averaged over samples.
    Higher BI = more important layer.
    """
    model.eval()
    ...
    with torch.inference_mode():
        for s in range(num_samples):
            ...
            for i in range(model.num_encoder_layers):
                x_before = x.clone()
                ve = model._get_ve(i, input_ids)
                x = model.blocks[i](x, x0, v_embed=ve)
                skips.append(x)
                # BI = 1 - cosine_similarity
                cos = F.cosine_similarity(x_before.flatten(), x.flatten(), dim=0).item()
                bi_scores[i] += (1.0 - cos)
            ...
            for i in range(model.num_decoder_layers):
                ...
                x_before = x.clone()
                ve = model._get_ve(bi, input_ids)
                x = model.blocks[bi](x, x0, v_embed=ve)
                cos = F.cosine_similarity(x_before.flatten(), x.flatten(), dim=0).item()
                bi_scores[bi] += (1.0 - cos)

    # Average over samples
    actual_samples = min(num_samples, max(1, (val_tokens.numel() - 1) // seq_len))
    bi_scores = [s / actual_samples for s in bi_scores]
    return bi_scores
...
def prune_model_inplace(
    ...
    """Remove layers IN-PLACE using ShortGPT-style del.
    If force_first=True, remove layers [0, 1] instead of BI-lowest.
    Updates model.blocks, num_encoder/decoder_layers, skip_weights.
    Returns list of removed layer indices.
    """
    num_layers = len(model.blocks)
    if force_first:
        # Remove the first 2 layers (reverse order for safe deletion)
        layers_to_remove = [1, 0]
    else:
    ...
    while True:
        ...
        if args.prune_enabled and not pruning_done and args.prune_layers > 0:
            ...
            if approx_ms >= prune_wallclock_ms:
                ...
                # 2. Prune in-place
                removed = prune_model_inplace(base_model, bi_scores, args.prune_layers, device, force_first=args.prune_first_layers)
