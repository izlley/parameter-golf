...
def quantize_saliency_mixed_per_row(t: Tensor, top_pct: float = 0.01, low_bits: int = 4) -> tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    """
    t32 = t.float()
    # Compute per-element saliency (magnitude-based)
    saliency = t32.abs()
    # Find threshold for top_pct across entire tensor
    k = max(1, int(t32.numel() * top_pct))
    threshold = torch.topk(saliency.reshape(-1), k, largest=True).values[-1].item()
    # Salient mask
    mask = saliency >= threshold
    # Limit to exactly k elements (in case of ties)
    if mask.sum() > k:
        indices = torch.nonzero(mask.reshape(-1), as_tuple=False).squeeze(-1)
        perm = torch.randperm(indices.numel())[:k]
        mask_flat = torch.zeros(t32.numel(), dtype=torch.bool)
        mask_flat[indices[perm]] = True
        mask = mask_flat.reshape(t32.shape)
    # Store salient weights as fp16
    salient_indices = torch.nonzero(mask.reshape(-1), as_tuple=False).squeeze(-1).to(torch.int32)
    salient_vals = t32.reshape(-1)[salient_indices].to(torch.float16)
    # Zero out salient weights before quantizing the rest
    t_masked = t32.clone()
    t_masked[mask] = 0.0
    # Quantize non-salient weights to low_bits
    clip_range = (1 << (low_bits - 1)) - 1  # int4: 7, int5: 15
    if t_masked.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        for pct in [0.9990, 0.9995, 0.9999, 1.0]:
            if pct < 1.0:
                row_clip = torch.quantile(t_masked.abs(), pct, dim=1)
            else:
                row_clip = t_masked.abs().amax(dim=1)
            s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
            q = torch.clamp(torch.round(t_masked / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            recon = q.float() * s.float()[:, None]
            # Add back salient values for error computation
            recon_full = recon.clone()
            recon_full[mask] = t32[mask]
            err = (t32 - recon_full).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q, s, err
        return best_q, best_s, salient_vals, salient_indices
    # 1D fallback
    amax = t_masked.abs().max().item()
    scale = torch.tensor(amax / clip_range if amax > 0 else 1.0, dtype=torch.float16)
    q = torch.clamp(torch.round(t_masked / scale.float()), -clip_range, clip_range).to(torch.int8)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if saliency_cats and cat in saliency_cats and t.ndim == 2:
            q, s, sv, si = quantize_saliency_mixed_per_row(t, saliency_top_pct, saliency_low_bits)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            result[name + ".sv"] = sv  # salient values
            result[name + ".si"] = si  # salient indices
            meta[name] = {"type": "saliency", "shape": list(t.shape)}
            continue
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        if isinstance(info, dict) and info.get("type") == "saliency":
            q, s = result[name + ".q"], result[name + ".scale"]
            sv, si = result[name + ".sv"], result[name + ".si"]
            orig_shape = tuple(info["shape"])
            # Dequantize low-precision part
            if s.ndim > 0:
                t_out = (q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1))))
            else:
                t_out = q.float() * float(s.item())
            # Restore salient values
            t_flat = t_out.reshape(-1)
            t_flat[si.long()] = sv.float()
            out[name] = t_flat.reshape(orig_shape).to(orig_dtype)
            continue
        q, s = result[name + ".q"], result[name + ".scale"]
        if s.ndim > 0:
...
def main() -> None:
    ...
    saliency_cats = {"mlp", "attn"} if args.saliency_top_pct > 0 else None
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu, {"mlp", "attn"},
        saliency_cats=saliency_cats,
        saliency_top_pct=args.saliency_top_pct,
        saliency_low_bits=args.saliency_low_bits,
    )
