...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float, rope_base: float, qk_gain_init: float,
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, kv_pool_stride=kv_pool_stride)
...
class GPT(nn.Module):
    def __init__(
        ...
        # KV pooling for encoder layers (first half): stride=2 to reduce attention cost
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                      kv_pool_stride=2 if i < self.num_encoder_layers else 1)
