...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    # EWQ: early layers (0-3) use int5, rest use int6
    int5_layers = set(range(min(4, num_layers_total)))
...
def main() -> None:
    ...
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
    # EWQ: mark early layer CastedLinear modules for int5 QAT (layers 0-3)
    for i in range(min(4, len(base_model.blocks))):
    ...
    # Magnitude pruning 8%: zero out smallest weights for better compression
    prune_frac = 0.08
    for name, param in sd_cpu.items():
        if param.ndim == 2 and param.numel() > 65536:
            threshold = torch.quantile(param.float().abs(), prune_frac)
            param[param.abs() < threshold] = 0.0
    log0(f"magnitude_pruning: {prune_frac*100:.0f}% applied to large 2D tensors")
