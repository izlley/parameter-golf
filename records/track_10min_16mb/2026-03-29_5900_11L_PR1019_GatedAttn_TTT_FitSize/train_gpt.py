...
class MuonLiteTTT(torch.optim.Optimizer):
    """Sign-based updates with Nesterov momentum for TTT mask training."""
...
def eval_val_sliding_ttt(
    ...
    """Supermask TTT adapted for Parameter Banking architecture.
    Freeze all model weights, learn per-MLP + per-Attn channel masks.
    ...
    # --- Supermask setup adapted for Parameter Banking ---
    # Freeze ALL model weights
    for p in base_model.parameters():
        p.requires_grad_(False)

    # Determine dimensions from bank tensors
    n = base_model.num_layers
    mlp_hidden_dim = base_model.mlp_up_bank.shape[1]  # mlp_up_bank: (n, mlp_dim, model_dim)
    model_dim = base_model.qo_bank.shape[1]  # qo_bank: (2n, model_dim, model_dim)
    ...
    for bi, block in enumerate(base_model.blocks):
        # MLP mask: per hidden channel (init=3.0 -> sigmoid~0.953)
        mlp_ms = torch.full((mlp_hidden_dim,), 3.0, device=device, requires_grad=True)
        mlp_bias = torch.zeros(mlp_hidden_dim, device=device, requires_grad=True)
        # Attn mask: per model_dim channel (init=3.0 -> sigmoid~0.953)
        attn_ms = torch.full((model_dim,), 3.0, device=device, requires_grad=True)
        attn_bias = torch.zeros(model_dim, device=device, requires_grad=True)
    ...
    # Monkey-patch forwards with masked+biased versions for Parameter Banking
    for bi, block in enumerate(base_model.blocks):
        ...
        block.mlp.forward = _make_masked_mlp_fwd(block.mlp, mlp_ms, mlp_bias)
        ...
        def _make_masked_attn_fwd(attn_module, ms, bias, orig_fwd):
            def masked_attn_forward(x, q_w, k_w, v_w, out_w, v_embed=None, v0=None):
                out, raw_v = orig_fwd(attn_module, x, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
    ...
    # Restore original forwards and requires_grad
...
def main() -> None:
    ...
    # NOTE: target is in MiB (×1024²), but contest limit is 16,000,000 decimal bytes
    # 15.9 MiB = 16.67MB decimal (OVER LIMIT). Use 15.15 MiB ≈ 15.88MB decimal for safety.
    target_mb = float(os.environ.get("TARGET_MB", "15.15"))
