...
def quantize_int5_per_row(t: Tensor, clip_range: int = 15) -> tuple[Tensor, Tensor]:
    """Int5 quantization: [-15, 15] range for less sensitive tensors."""
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
    ...
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    return q, scale
def compute_tensor_sensitivity(state_dict: dict[str, Tensor], int6_cats: set[str]) -> dict[str, float]:
    """Compute per-tensor quantization sensitivity: weighted MSE(int5) - MSE(int6)."""
    sensitivity = {}
    for name, tensor in state_dict.items():
        ...
        # Sensitivity = MSE degradation * num_elements (size-weighted)
        sensitivity[name] = (err5 - err6) * tensor.numel()
    return sensitivity
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    # Compute per-tensor sensitivity
    sensitivity = compute_tensor_sensitivity(state_dict, int6_cats)
    # Sort by sensitivity (ascending = least sensitive first)
    sorted_tensors = sorted(sensitivity.items(), key=lambda x: x[1])
    # Select least sensitive tensors for int5 until we cover ~55% of quantizable params
    total_params = sum(state_dict[name].numel() for name, _ in sorted_tensors)
    target_int5_params = int(total_params * 0.55)  # ~6/11 layers worth
    cumulative = 0
    int5_tensors = set()
    for name, sens in sorted_tensors:
        if cumulative >= target_int5_params:
            break
        int5_tensors.add(name)
        cumulative += state_dict[name].numel()
    print(f"PerTensor-EWQ: {len(int5_tensors)}/{len(sensitivity)} tensors selected for int5, "
...
def main() -> None:
    ...
    # EWQ: mark early layer CastedLinear modules for int5 QAT (heuristic)
