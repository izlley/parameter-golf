...
class Hyperparameters:
    ...
    ttt_target_modules = os.environ.get("TTT_TARGET_MODULES", "c_q,c_v")
...
class CastedLinear(nn.Linear):
    _qat: bool = False
    _qat_clip: int = 31  # default int6; set to 15 for int5 (MLP)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._qat and self.training and w.ndim == 2:
            ...
            scale = amax / clip
            q = (w_f / scale).round().clamp(-(clip + 1), clip)
            w_q = q * scale
...
# -----------------------------
# LoRA for Test-Time Training
# -----------------------------

class LoRALinear(nn.Module):
    """Low-rank adapter wrapping an existing linear layer."""
    def __init__(self, base_linear: nn.Linear, rank: int):
        super().__init__()
        self.base_linear = base_linear
        in_f = base_linear.in_features
        out_f = base_linear.out_features
        self.lora_a = nn.Parameter(torch.randn(in_f, rank, dtype=torch.float32) * 0.01)
        self.lora_b = nn.Parameter(torch.zeros(rank, out_f, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        base_out = self.base_linear(x)
        lora_out = (x.float() @ self.lora_a @ self.lora_b).to(dtype=base_out.dtype)
        return base_out + lora_out


def inject_lora(model: nn.Module, target_names: list[str], rank: int, device: torch.device) -> list[tuple[nn.Module, str, nn.Module]]:
    """Inject LoRA adapters into target modules. Returns list of (parent, attr, original) for removal."""
    injected: list[tuple[nn.Module, str, nn.Module]] = []
    for name, module in list(model.named_modules()):
        ...
        attr = parts[-1]
        lora = LoRALinear(module, rank).to(device)
        setattr(parent, attr, lora)
        injected.append((parent, attr, module))
    return injected


def remove_lora(injected: list[tuple[nn.Module, str, nn.Module]]) -> None:
    """Remove LoRA adapters and restore original linear layers."""
    for parent, attr, original in injected:
        setattr(parent, attr, original)


def ttt_adapt_and_eval(
    args: Hyperparameters,
    ...
    """Run TTT adaptation on a batch, return per-position NLL."""
    seq_len = args.train_seq_len

    # Inject LoRA
    target_names = [t.strip() for t in args.ttt_target_modules.split(",")]
    injected = inject_lora(base_model, target_names, args.ttt_rank, device)

    # Collect LoRA params
    lora_params = []
    for parent, attr, _ in injected:
        lora_mod = getattr(parent, attr)
        lora_params.extend([lora_mod.lora_a, lora_mod.lora_b])

    # TTT gradient steps on the batch itself (self-supervised)
    base_model.train()
    optimizer = torch.optim.Adam(lora_params, lr=args.ttt_lr)
    ...
    with torch.inference_mode():
        ...
        nll = F.cross_entropy(
            ...
            y_batch.reshape(-1),
            reduction="none",
        ).reshape(x_batch.shape[0], seq_len)

    # Remove LoRA and restore
    remove_lora(injected)
    base_model.eval()
...
def eval_val_sliding(
    ...
    use_ttt = getattr(args, "ttt_enabled", False)
    base_model.eval()
    ...
    for bi in range(0, len(my_windows), batch_seqs):
        ...
        if use_ttt:
            # TTT needs gradients — cannot use inference_mode
            nll = ttt_adapt_and_eval(args, base_model, x_batch, y_batch, wlens, device)
        else:
            with torch.inference_mode():
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    logits = base_model.forward_logits(x_batch)
...
def main() -> None:
    ...
    if master_process:
        ...
        if _HAS_WANDB:
            wandb.init(
                ...
                name=f"seed{args.seed}_{args.run_id[:8]}",
    ...
    # Enable QAT BEFORE torch.compile to avoid graph breaks / recompilation
    if args.qat_enabled:
        for name, module in base_model.named_modules():
            if isinstance(module, CastedLinear):
                module._qat = True
                module._qat_clip = 15 if ".mlp." in name else 31  # int5 MLP, int6 attn
        if master_process:
            log0("qat:enabled before compile (int5 MLP, int6 attn)")
    ...
    while True:
        ...
        if should_log_train:
            ...
            if master_process and _HAS_WANDB:
                wandb.log({"train_loss": tl, "step_avg_ms": approx_training_time_ms / step, "lr_scale": scale}, step=step)
    ...
    if args.eval_stride > 0 and args.eval_stride < args.train_seq_len:
        ttt_tag = f" ttt_rank:{args.ttt_rank} ttt_steps:{args.ttt_steps} ttt_lr:{args.ttt_lr}" if args.ttt_enabled else ""
        log0(f"final_eval_mode:sliding_window stride:{args.eval_stride} batch_seqs:{args.eval_batch_seqs}{ttt_tag}")
