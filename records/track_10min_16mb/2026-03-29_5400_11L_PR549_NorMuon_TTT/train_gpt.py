...
class MuonLiteTTT(torch.optim.Optimizer):
    """Lightweight Muon-inspired optimizer for TTT mask/bias parameters."""
...
def eval_val_sliding_ttt(
    ...
    """SupermaskTTT: freeze all model weights, learn masks+biases on MLP hidden
    and attention output channels. Score-first legality."""
    ...
    # Create learnable masks + biases per block
    n = base_model.num_layers
    mlp_dim = base_model.mlp_up_bank.shape[1]  # hidden dim of MLP
    model_dim = base_model.mlp_down_bank.shape[1] if base_model.mlp_down_bank.ndim == 3 else base_model.qo_bank.shape[1]
    ...
    for i in range(n):
        # MLP: mask on hidden activations (after leaky_relu().square())
        mm = nn.Parameter(torch.ones(mlp_dim, device=device, dtype=torch.float32))
        mb = nn.Parameter(torch.zeros(mlp_dim, device=device, dtype=torch.float32))
        mlp_masks.append(mm)
        mlp_biases.append(mb)
        mask_params.extend([mm, mb])
        # Attn: mask on output (model_dim channels)
        am = nn.Parameter(torch.ones(model_dim, device=device, dtype=torch.float32))
        ab = nn.Parameter(torch.zeros(model_dim, device=device, dtype=torch.float32))
        attn_masks.append(am)
        attn_biases.append(ab)
        mask_params.extend([am, ab])

    total_mask_params = sum(p.numel() for p in mask_params)
    log0(f"supermask_ttt:mask_params={total_mask_params}")

    # Save original forward methods
    orig_mlp_forwards = [block.mlp.forward for block in base_model.blocks]
    orig_attn_forwards = [block.attn.forward for block in base_model.blocks]

    # Monkey-patch MLP forwards with mask
    for i, block in enumerate(base_model.blocks):
        mm_i = mlp_masks[i]
        mb_i = mlp_biases[i]
        def make_masked_mlp(mm, mb):
            def masked_forward(x, up_w, down_w):
                h = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5).square()
                # Apply mask + bias on hidden activations
                h = h * mm.to(dtype=h.dtype)[None, None, :] + mb.to(dtype=h.dtype)[None, None, :]
                return F.linear(h, down_w.to(x.dtype))
            return masked_forward
        block.mlp.forward = make_masked_mlp(mm_i, mb_i)

    # Monkey-patch Attn forwards with mask on output
    for i, block in enumerate(base_model.blocks):
        am_i = attn_masks[i]
        ab_i = attn_biases[i]
        orig_attn = orig_attn_forwards[i]
        def make_masked_attn(am, ab, orig_fn):
            def masked_forward(x, q_w, k_w, v_w, out_w, v_embed=None, v0=None):
                out, raw_v = orig_fn(x, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
                # Apply mask + bias on attention output
                out = out * am.to(dtype=out.dtype)[None, None, :] + ab.to(dtype=out.dtype)[None, None, :]
                return out, raw_v
            return masked_forward
        block.attn.forward = make_masked_attn(am_i, ab_i, orig_attn)

    optimizer = MuonLiteTTT(mask_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for j, ws in enumerate(batch_ws):
                    ...
                    x_batch[j, :wlen] = chunk_tok[:-1]
                    y_batch[j, :wlen] = chunk_tok[1:]
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                for j, ws in enumerate(batch_ws):
                    ...
                    loss_sum += scored_nll.sum()
                    token_count += float(wlen - s)
                    tgt, prev = y_batch[j, s:wlen], x_batch[j, s:wlen]
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        torch.nn.utils.clip_grad_norm_(mask_params, args.ttt_grad_clip)
                        optimizer.step()
    ...
    for i, block in enumerate(base_model.blocks):
        block.mlp.forward = orig_mlp_forwards[i]
