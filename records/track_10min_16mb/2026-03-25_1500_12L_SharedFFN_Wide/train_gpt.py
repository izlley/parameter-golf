...
class Hyperparameters:
    ...
    model_dim = int(os.environ.get("MODEL_DIM", 656))
    ...
    shared_ffn_groups = int(os.environ.get("SHARED_FFN_GROUPS", 3))  # number of shared MLP groups (0=no sharing)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Shared FFN: create shared MLP groups and mark blocks to use them
        self.shared_ffn_groups = shared_ffn_groups
        use_shared = shared_ffn_groups > 0
        if use_shared:
            self.shared_mlps = nn.ModuleList([MLP(model_dim, mlp_mult) for _ in range(shared_ffn_groups)])
            # Map each layer to its shared MLP group index
            layers_per_group = num_layers // shared_ffn_groups
            self._layer_to_mlp_group = []
            for i in range(num_layers):
                group_idx = min(i // layers_per_group, shared_ffn_groups - 1)
                self._layer_to_mlp_group.append(group_idx)
        else:
    ...
    def _get_shared_mlp(self, layer_idx: int) -> nn.Module | None:
        """Return the shared MLP for this layer, or None if not using shared FFN."""
        if not self._layer_to_mlp_group:
            return None
        return self.shared_mlps[self._layer_to_mlp_group[layer_idx]]
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_ffn_groups=args.shared_ffn_groups,
    ).to(device).bfloat16()
    ...
    # Add shared MLP params to optimizer groups
    if base_model.shared_mlps:
        for name, p in base_model.shared_mlps.named_parameters():
