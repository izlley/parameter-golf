...
class GPT(nn.Module):
    def __init__(
        ...
        # MoR: Layer 9 and 10 share the same block (recursive, no skip conflict)
        self.mor_layer_idx = 9  # Layer 9 is the recursive block
        self.mor_max_repeat = 2
        self.blocks[self.mor_layer_idx + 1] = self.blocks[self.mor_layer_idx]  # Share block 10 = block 9
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if bi == self.mor_layer_idx:
                # MoR iter1: apply skip (from encoder 0), run shared block
                if skips:
                ...
                # MoR iter2: no skip (already consumed), recursion on iter1 output
                ve2 = self._get_ve(bi + 1, input_ids, ve_cache)
                x_iter2 = self.blocks[bi](x_iter1, x0, v_embed=ve2)
                ...
                # Skip: already handled by MoR above (layer 10 is same block as layer 9)
                pass
            else:
                if skips:
                    x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
