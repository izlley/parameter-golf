        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    # Only apply to params tagged with _normuon=True (later layers)
                    if getattr(p, '_normuon', True):
...
def main() -> None:
    ...
    # Mixed optimizer: tag block params for NorMuon (layers >= 5) vs standard Muon (layers 0-4)
    normuon_layer_start = 5
    # Build a set of param ids from block_named_params with their layer indices
    _block_param_layer = {}
    for name, p in block_named_params:
        parts = name.split(".")
        if len(parts) > 0 and parts[0].isdigit():
            _block_param_layer[id(p)] = int(parts[0])
    for p in matrix_params:
        layer_idx = _block_param_layer.get(id(p), -1)
        p._normuon = (layer_idx >= normuon_layer_start)  # non-block params (layer_idx=-1) get standard Muon
    optimizer_muon = Muon(
        matrix_params,
        lr=args.matrix_lr,
