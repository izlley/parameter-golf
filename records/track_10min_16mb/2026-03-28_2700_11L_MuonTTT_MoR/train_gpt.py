...
class DepthRouter(nn.Module):
    """Lightweight router for MoR: decides per-token recursion depth."""
...
class GPT(nn.Module):
    def __init__(
        ...
        # MoR: share block 8 and 9 (deep decoder layers)
        self.mor_layer_idx = 8
        self.blocks[self.mor_layer_idx + 1] = self.blocks[self.mor_layer_idx]
        self.depth_router = DepthRouter(model_dim, hidden=64)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if bi == self.mor_layer_idx:
                ...
                x_iter1 = x_iter1 * self.iter_scales[0].to(dtype=x_iter1.dtype)[None, None, :]
                # Route: per-token depth decision
                route_prob = self.depth_router(x_iter1)
                # MoR iteration 2
                ve2 = self._get_ve(bi, input_ids, ve_cache)
                x_iter2 = self.blocks[bi](x_iter1, x0, v_embed=ve2)
                x_iter2 = x_iter2 * self.iter_scales[1].to(dtype=x_iter2.dtype)[None, None, :]
                # Blend based on router
                x = route_prob * x_iter2 + (1 - route_prob) * x_iter1
            elif bi == self.mor_layer_idx + 1:
                pass  # skip: already handled by MoR above
            else:
                ve = self._get_ve(bi, input_ids, ve_cache)
...
def main() -> None:
    ...
    # MoR router and iter_scales
    scalar_params.extend(list(base_model.depth_router.parameters()))
    for s in base_model.iter_scales:
        scalar_params.append(s)
